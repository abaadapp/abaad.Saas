...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $employee = User::create([
            ...
            'status' => 'نشط',
            /*
             * ولا صورة: موظّفٌ لم تُرفع صورتُه لا صورة له.
             *
             * كان يُكتب هنا رابطُ `picsum.photos` — خدمةُ صورٍ عشوائيّة على
             * الإنترنت — فيُولد الموظّف بوجهِ إنسانٍ آخر في ملفّه وفي شارته
             * وفي شاشة اختيار الكاشير. ولا يفرّق أحدٌ بينه وبين صورةٍ رُفعت،
             * لأنّه يُخزَّن في العمود نفسه.
             *
    ...
    public function update(Request $request, $id)
    {
         ...
         *
         * `Roles::STAFF` تُخرج `admin` من أدوار الوظائف عمدًا — «وظيفةٌ تمنحه
         * لأيّ موظّف تمنحه كلَّ شيء». والقاعدةُ كانت نصفَ قاعدة: لا تكتبه،
         * لكنّها **تمحوه**.
         *
         * وعلى الإنتاج يحمل صاحبا متجرين وظيفةَ «مدير» ودورُها `manager`،
         * وصفُّهما `admin`. فيفتح أحدُهما بطاقتَه ليصحّح رقمَ هاتفه، فيُكتب
         * دورُ الوظيفة في صفّه: يصير مديرَ فرع. قِسناه: ٣٠٢ ودورُه صار
         * `manager`.
         *
         * ولا شيء يبدو معطوبًا بعدها — `manager` تملك `'*'` كما يملك — لكنّه
         * يفقد ما لا يُستعاد: `Permissions::isOwner` تصير كاذبةً عليه فيُقاس
         * كما يُقاس موظّف، ويفقد `invoice.override`، **ولا يبقى في المتجر
         * صفٌّ دورُه `admin`**. ولا شاشةَ في لوحة التاجر تُعيده: كلُّ وظيفةٍ
         * تُقيَّد بـ`Roles::STAFF` وليس فيها `admin`. بابٌ يُغلق من داخله.
         *
         * فيُترك دورُه كما هو، ويمضي باقي الحفظ. والوظيفةُ اسمٌ يُعرض، وصفةُ
         * صاحب النشاط ليست اسمًا يُعرض.
         */
