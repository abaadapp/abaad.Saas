...
class PageController extends Controller
{
    ...
    public function employeesShow(string $id): Response
    {
        ...
        return Inertia::render('Admin/Employees/Show', [
            ...
            /*
             * وأزرارُ «تعطيل» و«إعادة تعيين كلمة المرور» و«تعديل» تُرسم على
             * هذا لا على الأمنيات.
             *
             * الحارسُ يردّ من يمسّ حسابًا يفتح أكثر ممّا يفتح، والشاشةُ كانت
             * ترسمها للجميع — فيضغطها الموظّف فيُردّ بـ٤٠٣ لا يقول شيئًا.
             * وبابٌ معروضٌ لا يُفتح أسوأ من بابٍ لا يُعرض.
             */
            'may_touch' => $user !== null && Permissions::mayTouch(auth()->user(), $user),
        ]);
    }
        ...
        return Inertia::render('Admin/Settings/Index', [
            ...
            'store' => [
                ...
                'readiness' => $b?->storefrontTheme() !== null
                    ...
                    : null,
                /*
                 * ومن لبس واجهةً خاصّة يُحرّر صفحتَه في محرّرها — لا هنا.
                 *
                 * و`omit` ليست قائمةَ ما يُخفى بل قائمةَ ما **لا تُرسله**
                 * هذه البطاقة: نموذجُ Inertia يلتقط قيمَه حين تُفتح الشاشة،
                 * فمقبضٌ مخفيٌّ يبقى في الحمولة يحمل ما كان قبل أن يُفتح
                 * المحرّر — ويكتب حفظُ رسمِ توصيلٍ فوق ما رتّبه في صفحته
                 * قبل دقيقة، بلا خطأٍ ولا رسالة. انظر `Store\PageEditor`.
                 */
                'themed' => $b?->storefrontTheme() !== null
