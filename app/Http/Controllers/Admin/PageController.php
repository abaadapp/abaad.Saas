...
class PageController extends Controller
{
    ...
    /**
     * فهرس التقارير — أرقامُ المتجر أوّلًا، ثمّ أبوابُ التفصيل.
     *
     ...
     *
     * والأرقام عادت إلى رأس الصفحة بعد أن غابت عنها.
     *
     * كان «التقارير» لوحةً تُفتح فتقول مبيعاتك وربحك ومصروفك ومنحنى بيعك،
     * فصار — حين أُفرد لكلّ تقريرٍ صفحته — حقلَ بحثٍ وخمسَ عشرةَ بطاقةً
     * رماديةً بلا رقمٍ واحد. والفَرقُ ليس ذوقًا: صاحبُ المحلّ يفتح «التقارير»
     * ليعرف كم باع، فصار عليه أن يعرف اسمَ التقرير الذي فيه ذلك ويضغطه.
     *
     * والمصدر واحدٌ لا نسخة: `Reports::salesReport` نفسها تُغذّي هذا الرأسَ
     * وصفحةَ «ملخّص المبيعات» والملفّاتِ الثلاثة — فلا يقول الرأسُ رقمًا
     * وتقول الصفحةُ غيره.
     */
    public function reportsIndex(Request $request): Response
    {
        $headline = Reports::salesReport($request->query('range'));

        return Inertia::render('Admin/Reports/Index', [
            'reports' => Reports::forUser(auth()->user()),
            'categories' => Reports::categoryLabels(),
            'summary' => $headline['summary'],
            'salesSeries' => $headline['salesSeries'],
