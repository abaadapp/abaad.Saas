...
class PageController extends Controller
{
    ...
    private const SETTING_DEFAULTS = [
        ...
        /*
         * ═══ وما بعد الحصّة يُباع ═══
         *
         * حجمُ الحزمة وثمنُها بالريال — يقرؤهما `WhatsAppPacks` حين يطلب
         * تاجرٌ حزمة، ويُنسخان في صفّ الطلب ساعتَها. فتعديلُ السعر هنا يسري
         * على ما يُطلب بعده، ولا يُغيّر ثمنَ طلبٍ قائم.
         *
         * والرقمان حرفًا لا مُشتقَّين من `WhatsAppPacks::FALLBACK_*`: تعبيرٌ
         * كهذا لا يقبله PHP 8.4 داخل `const` — انظر السطر أعلاه. والتطابقُ
         * محروسٌ باختبارٍ يقارنهما.
         */
        'whatsapp_pack_size' => '500',
        'whatsapp_pack_price' => '5.000',
        /*
        ...
        /*
         * ═══ ومقابضُ الأرشفة — وكانت الشاشةُ تعرضها ولا تصلها ═══
         *
         * `platformSettings` تقرأ من القاعدة ما كان مفتاحُه في هذا الثابت
         * وحدَه، ثمّ تفرشه فوقه. فمفتاحٌ ليس هنا لا يبلغ الشاشةَ أبدًا — لا
         * افتراضُه ولا ما حُفظ منه.
         *
         * وأثرُ ذلك قِيس على المستودع، وهو ثلاثةُ أشياء:
         *
         *  ١. حقولُ «مدة الاحتفاظ» و«الأسبوعيّة» و«أقصى حجم» تُرسم **فارغة**
         *     دائمًا، والنظامُ يعمل بـ١٢ و١٢ و٥٠٠. فيقرأ المشغّل فراغًا عن
         *     سياسةٍ قائمة — وتقريرُ حالٍ كاذب أسوأ من غياب التقرير.
         *
         *  ٢. ما يُحفظ لا يعود: يكتب ٢٤ شهرًا فيُخزَّن ويُقرأ، ثمّ يفتح
         *     الشاشةَ فيجد الحقلَ فارغًا كأنّه لم يكتب شيئًا.
         *
         *  ٣. وأسوأُها: النموذجُ واحدٌ لتبويبات الشاشة كلِّها. فحفظةٌ من
         *     تبويب «عامة» ترسل الحقلَ الفارغ معها، فتُمحى الأربعةُ
         *     والعشرون ويعود النظامُ إلى ١٢ **صامتًا**.
         *
         * والأرقامُ مكتوبةٌ حرفًا لا مُشتقّةً من `Archive\Policy`: ‏`const`
         * لا يقبل التحويل إلى نصّ في PHP 8.4 (انظر
         * `whatsapp_shared_default_monthly_limit` فوق). ويحرسها من
         * الافتراق عن ثوابتها اختبارٌ يقارنهما.
         */
        'archive_enabled' => '1',
        'archive_manual_enabled' => '1',
        'archive_retention_months' => '12',
        'archive_weekly_enabled' => '1',
        'archive_weekly_retention_weeks' => '12',
        'archive_max_mb' => '500',
        // ولا قرصَ بعيدًا حتى يُعرَّف واحدٌ في config/filesystems.php
        'archive_remote_disk' => '',
