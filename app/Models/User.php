...
class User extends Authenticatable
{
    ...
    public function worksAt(?int $branchId): bool
    {
        ...
        /*
         * ═══ والفراغُ الذي يعني «كلّ الفروع» فراغٌ واحد: ألّا يُقيَّد أصلًا ═══
         *
         * `branches()` تقرأ عبر النموذج، والنموذجُ يُخفي المحذوفَ ليّنًا. فصفٌّ
         * قيّده بـ«الخوير» وحدها يقرأ فارغًا يوم يُحذف الخوير — وحالتان
         * مختلفتان تمامًا تقعان على جوابٍ واحد:
         *
         *   «لم يُقيَّد» → يعمل في كلّ فرع (وهو الصواب، ونصُّ الفقرة أعلاه)
         *   «قُيِّد بفرعٍ زال» → **صار يعمل في كلّ فرع**
         *
         * قِسناه: كاشيرُ الخوير يُردّ عن صندوق السيب (٣٠٢)، فيُحذف الخوير،
         * فيفتح صندوق السيب (٢٠٠) **ويبيع عليه** — والفاتورة تُكتب على فرعٍ
         * لم يُسنَد إليه يومًا. ويشمل ذلك فرعًا يُفتح بعد الحذف بشهر.
         *
         * فالقيدُ يُقاس بصفوفه الخام — بالمحذوف فيها — ويُفشل مغلقًا: من
         * قُيِّد بفرعٍ زال لا يعمل حتى يُسنَد من جديد. ويُقال له ولمديره:
         * الكاشيرُ يقرأ «هذا الجهاز يعمل على فرعٍ لست مُسنَدًا إليه» عند
         * الباب، والمديرُ يُخبَر ساعةَ يحذف الفرع — انظر `BranchController::destroy`.
         *
         * والصفُّ يبقى ولا يُمحى: إن استُعيد الفرعُ من سلّة المحذوفات عاد
         * إسنادُه كما كان.
         */
        return $this->branches()->withTrashed()->count() === 0;
    }
    ...
    /**
     * هل هذا موظّفٌ في متجر؟ — لا صاحبُه ولا مديرُ المنصّة.
     *
     * ═══ ولمن يُعرض «حسابي وراتبي» ═══
     *
     * صفحةٌ فيها راتبُ صاحبها ومسيراتُه ومبيعاتُه. وشرطُ عرضها في الشريط
     * كان «دورُه ليس `admin`» — ومديرُ المنصّة ليس `admin`، فيُرسم له
     * البند. فإن ضغطه قذفه `RequiresBusiness` إلى لوحة المنصّة برسالةٍ عن
     * «لوحة النشاط تخصّ متجرًا بعينه» — لا عن راتب. بابٌ مرسومٌ لا يؤدّي
     * إلى شيء، وصاحبُه يظنّ العطبَ في النظام.
     *
     * ومديرُ الفرع (`manager`) موظّفٌ: له راتبٌ ومسيرة.
     *
     * ═══ ولماذا لا يُحرَس صاحبُ النشاط في الخادم ═══
     *
     * البندُ يُخفى عنه — ملفُّه في الغالب ليس ملفَّ موظّف — لكنّ المسار
     * يبقى مفتوحًا له عمدًا: `PayrollRunController::payableEmployees` لا
     * ترشّح بالدور، وشاشةُ الموظفين تستثني `super_admin` وحده. فصاحبُ نشاطٍ
     * سجّل لنفسه راتبًا **يدخل المسيرة فعلًا وله سطرٌ فيها** — وحجبُ
     * الصفحة عنه يُخفي عنه مسيرتَه هو.
     *
     * ومديرُ المنصّة لا يحتاج حارسًا هنا: لا `business_id` له،
     * فـ`RequiresBusiness` تردّه قبل أن يبلغ المتحكّم — قِسناه: ٣٠٢.
     * فالعطبُ كان في الشاشة وحدها، وهنا يُصلَح.
     */
    public function isEmployee(): bool
    {
        return $this->business_id !== null && $this->role !== 'admin';
    }
