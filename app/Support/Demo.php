...
class Demo
{
    ...
    /**
     * الفرعُ المختار — إن كان ما يزال فرعًا في هذا المتجر.
     *
     * ═══ ولمَ يُفحص هنا لا في كلّ قارئ ═══
     *
     * الاختيارُ يُحفظ في الجلسة رقمًا، والجلسةُ تعمّر أكثر من الفرع: يُحذف
     * الفرعُ من هذه الشاشة أو من تبويبٍ آخر، ويبقى الرقم.
     *
     * فكان `currentBranchName` تفحص وتقول «كل الفروع»، و`currentBranchId`
     * تُعيد الرقمَ كما هو فيُرشَّح به كلُّ استعلام. فيقرأ التاجرُ ترويسةً
     * تقول «كل الفروع» فوق لوحةٍ فارغة: لا مبيعات ولا مخزون ولا طلبات —
     * لأنّ كلَّ سؤالٍ يُطرح عن فرعٍ لا وجود له. ولا شيءَ يقول ما جرى.
     *
     * والفحصُ هنا لأنّ هذه هي المنبع: منها تُشتقّ `activeBranchId`
     * و`currentBranchName` معًا، فلا يفترق قارئان لسؤالٍ واحد.
     *
     * ويُحصر بالمتجر كذلك: جلسةٌ قديمة تحمل فرعَ متجرٍ آخر لا تُرشِّح به.
     */
    public static function currentBranchId(): ?int
    {
        $id = (int) session('current_branch');

        // ولا تحرس هذه سلوكًا — `whereKey(0)` تردّ فارغًا على كلّ حال. تمنع
        // استعلامًا في أكثر الحالات شيوعًا: «كل الفروع»، وهو اللاشيء في الجلسة.
        if ($id <= 0) {
            return null;
        }
         ...
         * ولا ذاكرةَ هنا.
         *
         * `once()` تُفهرس بموضع النداء لا بقيمة `$id`: فلو بُدِّل الفرعُ في
         * الطلب نفسِه لَقرأت جوابَ الفرع الأوّل. وثمنُ الاستعلام لا يُذكر —
         * مفتاحٌ أساسيّ على جدولٍ صفوفُه آحاد — وثمنُ جوابٍ قديمٍ شاشةٌ
         * تُرشَّح بفرعٍ غير الذي اختاره التاجر.
         */
        return Branch::whereKey($id)->where('business_id', self::bid())->exists() ? $id : null;
    }
    ...
    public static function adminStats(): array
    {
         ...
         *
         * كان `مبيعات الشهر − مصروفات الشهر`: بلا تكلفةِ بضاعةٍ أصلًا،
         * وبالضريبة داخلَ الإيراد. فمحلٌّ باع بألفٍ اشتراه بستّمئة وأنفق
         * مئتين يقرأ «صافي ربح ٨٠٠» وربحُه مئتان — أربعةُ أضعاف، على أوّل
    ...
    public static function orders(?Request $filter = null, ?int $limit = null): array
    {
        ...
        /*
         * والحدُّ في القاعدة لا في PHP.
         *
         * كانت اللوحة تقرأ كلَّ طلبٍ مباعٍ في المتجر ثمّ تقتطع ستّةً: ستّمئة
    ...
    public static function orderDetails($number): array
    {
        ...
        $items = $o->items->map(fn ($it) => [
             ...
             * وهذه الدالّةُ — مصدرُها الوحيد — لم تُرسلها قطّ. فسطرٌ يُرسم
             * لقيمةٍ لا تصل: «بلا سكر» يكتبها الكاشيرُ وتُطبع على الإيصال
            ...
            /*
             * ═══ وما اختاره الزبونُ يصل شاشةَ طلبه ═══
             *
             * كان البندُ المخصَّص يصل هذه الشاشةَ سطرًا واحدًا: «طلب مخصص»
             * وسعرُه. واللونُ والمقاسُ واسمُ المُهدى إليه — كلُّ ما سُئل عنه
             * الزبونُ ودُوّن — لا أثرَ له هنا.
             *
             * ويقرؤه **الورقُ** (`DocumentPaper`) و**لوحةُ التجهيز**
             * (`PreparationController`)، وتعمى عنه الشاشةُ التي يفتحها
             * صاحبُ المتجر حين يتّصل الزبون يقول «طلبتُ الأحمر». والبيانُ
             * في القاعدة طَوالَ الوقت.
             *
             * وبقارئ الورق نفسِه (`view`) — لا بحسابٍ ثالثٍ لسؤالٍ واحد.
             * والداخليُّ يظهر هنا خلافًا للورق: هذه شاشةُ صاحب المتجر، وتلك
             * ورقةُ زبونه.
             */
            'custom' => $it->isCustom() ? CustomArrangement::view($it->custom_details) : null,
        ])->all();
    ...
    public static function employees(?int $branchId = null): array
    {
        ...
        /*
         * ═══ ومبيعاتُه كلُّها تُحسب ولا تُقرأ من عمودٍ ميّت ═══
         *
         * كان `users.sales_total` — عمودٌ **لا يكتبه شيءٌ في النظام**: لا
         * بيعةٌ تزيده ولا ورديةٌ تُغلقه، ولا يملؤه إلا بذرةُ المتجر التجريبيّ.
         * فيقرأ كلُّ متجرٍ حقيقيّ صفرًا لكلّ موظّفٍ منذ فتح بابه — في بطاقة
         * «أداء الموظفين» على اللوحة، وفي عمود «المبيعات» في الإعدادات، وفي
         * «إجمالي المبيعات» على ملفّ الموظّف، وفي متوسّط الطلب المشتقّ منه.
         *
         * وأربعُ شاشاتٍ تعرض صفرًا لا تُخطئ أحدًا: لا رسالةَ عطبٍ ولا خانةَ
         * فارغة — رقمٌ صحيحُ الشكل كاذبُ المعنى.
         *
        ...
        return User::where('business_id', $bid)->where('role', '!=', 'super_admin')
            ->orderBy('id')->get()->map(function ($u) use ($monthly, $lifetime) {
                ...
                return [
                    ...
                    'phone' => $u->phone,
                    'email' => $u->email,
                    // ما باعه منذ التحق — لا عمودٌ لا يكتبه شيء
                    'sales' => (float) ($lifetime[$u->id] ?? 0),
                    'status' => $u->status,
                    'joined' => optional($u->created_at)->format('Y-m-d') ?? '—',
                    // مبيعاتُ الشهر — مبلغٌ لا نسبة. يُرتَّب عليه لوحُ الأداء
                    'achieved' => $achieved,
                    // هدفُه الشهريّ كما ضبطه التاجر — صفرٌ يعني «بلا هدف»
                    'target' => $target,
                    /*
                     * ═══ ونسبةُ التحقيق تُحسب هنا — أو لا تُقال ═══
                     *
                     * عمود «تحقيق الهدف» في قائمة الموظفين كان يرسم `achieved`
                     * ويُلحق به علامة `%`: مبلغًا بالريال يُقرأ نسبةً مئوية.
                     * فموظّفٌ باع واحدًا وعشرين ريالًا يُقال عنه «٢١٪»، وآخرُ
                     * باع ألفًا «١٬٠٠٠٪» — رقمٌ صحيحُ الشكل كاذبُ المعنى، ولا
                     * شيء في الشاشة يُخبر التاجر أنّ الوحدة ليست ما يظنّ.
                     *
                     * و`users.monthly_target` — الحقلُ الذي يملؤه التاجر في
                     * ملفّ الموظّف تحت «الهدف الشهري»، وتلميحُه يَعِد بأنّه
                     * «يُحتسب عليه تحقيقُ الهدف في قائمة الموظفين» — **لم يكن
                     * يُقرأ في النظام كلِّه**: يُكتب ويُعرض في نموذجه ثمّ لا
                     * يدخل حسابًا واحدًا. مقبضٌ لا يُدير شيئًا.
                     *
                     * فصار العمود يقول ما يعد به اسمُه. ولا يُقصَّ عند مئة:
                     * من تجاوز هدفه يُقرأ تجاوزُه — والقصُّ يخفي أحسنَ بائع.
                     *
                     * ومن لا هدف له لا نسبةَ له: `null` تُرسم شرطةً، لا صفرًا
    ...
    public static function purchaseOrders(): array
    {
         ...
         *
         * ═══ والمورّدُ يُقرأ مع الصفوف لا صفًّا صفًّا ═══
         *
         * `supplier_name` لقطةٌ تُكتب عند الإنشاء، وصفوفُ ما قبلها فارغةٌ
         * منها — فيسقط السطرُ إلى `$p->supplier` فيفتح استعلامًا لكلّ أمرٍ
         * قديم. لا يظهر في متجرٍ جديد كلُّ صفوفه تحمل الاسم، ويظهر عند من
         * له تاريخ: وهو وحده من عنده ما يكفي من الصفوف ليؤذيَه.
         */
        return PurchaseOrder::where('business_id', self::bid())->withCount('items')
            ->with(['items' => fn ($q) => $q->orderBy('id'), 'supplier:id,name'])
    ...
    public static function cogsFor(int $bid, ?\Illuminate\Support\Carbon $start, ?\Illuminate\Support\Carbon $end = null, ?int $branchId = null, ?string $channel = null): float
    {
         ...
         *
         * `Model::pluck` تبني نموذجًا لكلّ صفٍّ قبل أن تلتقط العمود — فمتجرٌ
     ...
     *
     * و`$branchId` للوحة وحدها: بطاقاتُها كلُّها مقيَّدةٌ بالفرع المختار،
     * وقائمةٌ لا تتبعه بينها تقول لصاحب فرعٍ إنّ أفضل ما يبيعه صنفٌ لا يوجد
     * عنده. والتقاريرُ تمرّ بلا فرعٍ فتبقى على معناها: المتجر كلُّه.
     *
    ...
    /**
     * كم يومًا يبقى التنبيهُ المُخفى مُخفًى.
     *
     * ═══ العطبُ الذي يعالجه ═══
     *
     * كان الإخفاءُ أبديًّا. ومفتاحُ تنبيه المخزون `low-<صنف>` ثابتٌ للصنف
     * نفسِه — فمن أخفى «مخزون منخفض: وردٌ أحمر» مرّةً **لا يُنبَّه عنه
     * أبدًا**: يُعيد التخزين، ينفد بعد شهرين، ويقف أمام زبونٍ يطلبه ولا
     * شيءَ قاله له. وطمأنينةٌ كاذبة أسوأ من تحذيرٍ كاذب.
     *
     * ومفاتيحُ أخرى تنمو بلا سقف — `daily-<تاريخ>` صفٌّ كلَّ يوم،
     * و`order-<رقم>` صفٌّ لكلّ طلبٍ يُخفى — فتُجمع صفوفٌ لا تُقرأ أبدًا.
     *
     * فصار «أخفِه» يعني «ليس الآن» لا «إلى الأبد»: ثلاثون يومًا، فإن بقي
     * السببُ قائمًا عاد التنبيه. وما زال سببُه قائمًا بعد شهرٍ يستحقّ أن
     * يُقال ثانيةً.
     *
     * والمدّةُ تُقاس عند **القراءة** لا عند التنظيف وحده: لو اتُّكل على
     * المجدول لَبقي الصنفُ صامتًا كلَّما تعثّر `trash:purge` ليلةً. والتنظيفُ
     * يمحو الصفوفَ بعدها — نظافةٌ لا سلوك.
     */
    public const DISMISSAL_DAYS = 30;

    /** مفاتيح التنبيهات التي أخفاها المستخدم الحالي — ما لم تنقضِ مدّتُها */
    public static function dismissedNotificationKeys(): array
    {
        ...
        return DismissedNotification::where('user_id', $u->id)
            ->where('created_at', '>=', now()->subDays(self::DISMISSAL_DAYS))
            ->pluck('notif_key')->all();
    }
    ...
    private static function buildNotifications(int $limit, bool $unfiltered = false): array
    {
        ...
        $add = function (string $key, array $item) use (&$items, $dismissed, $u, $unfiltered) {
            ...
            /*
             * وفعلٌ فوق القسم لمن أراد صفًّا أضيق.
             *
             * القسمُ يقول «يفتح هذه الشاشة»، وهو صوابٌ للصفّ الذي يقود
             * إليها. لكنّ صفًّا يوقظ أحدًا في الليل سؤالُه آخر: «هل هذا
             * عملُه؟». ومتجرٌ فيه عشرةٌ يفتحون «المبيعات» لا يريد عشرةَ
             * أجراسٍ تدقّ لطلبٍ واحد.
             *
             * والغيابُ يعني «لا فعلَ يحرسه» — فتبقى المصادرُ الاثنا عشر
             * كما كانت، ولا يُضيَّق على أحدٍ ما كان يصله.
             */
        ...
        if ($waAlert !== null) {
            $add('wa-delivery', [
                ...
                'color' => 'danger',
                /*
                 * ويقود إلى الصفوف نفسِها لا إلى شاشة الربط.
                 *
                 * كان يقود إلى «التطبيقات التكاملية ‹ واتساب» — وهي شاشةٌ
                 * تقول إنّ الرقم مربوطٌ وحصّتَك باقية، ولا تعرف أيَّ الثلاث
                 * فشلت ولا لمن ولا لماذا. فيُقال للتاجر إنّ شيئًا انكسر ولا
                 * يُقال ماذا، وبابٌ معروضٌ لا يُفتح أسوأ من بابٍ لا يُعرض.
                 */
                'url' => route('admin.marketing.whatsapp.log', ['filter' => 'failed']),
            ]);
        }

        /*
         * ولا تذكيرَ موسمٍ في الجرس — تنبيهُ الموسم في قسمه وحدَه.
         *
         * كان الجرسُ يعرضها، فصار للتاجر بابان لتنبيهٍ واحد: يُخفيه من الجرس
         * فيبقى في القسم، ويقرؤه في القسم فيبقى في الجرس. وموضعُ الاستعدادِ
         * للموسم شاشةُ المواسم لا قائمةٌ تجمع نقصَ المخزون وفشلَ الرسائل.
         *
         * فـ`Seasons::due` تُقرأ من `SeasonController::index` وحدَه، وبقيّةُ
         * سكّان الجرس على حالهم.
         */
        ...
        /*
         * ═══ وطلبُ الموقع يُقال إنّه من الموقع ═══
         *
         * كان الصفُّ واحدًا لكلّ ما ينتظر التجهيز — فطلبٌ أتمّه زبونٌ بنفسه
         * في الثانية بعد منتصف الليل يُقرأ بنصّ الطلب الذي كتبه الكاشير على
         * المنضدة قبل قليل: «طلب ١٠٤٢ بانتظار التجهيز». ولا شيء في الجرس
         * يقول إنّ أحدًا دفع من بيته وينتظر.
         *
         * والفرقُ ليس زينةً: بيعةُ المنضدة وقعت بحضور بائعها، وبيعةُ الموقع
         * تقع بلا أحد — فهي وحدها التي تحتاج أن تُوقظ من يجهّزها.
         *
         * وهي وحدها التي تُحرس بالفعل: بيعةُ الصندوق تبقى لكلّ من يفتح
         * «المبيعات» كما كانت، فلا يُقطع عن أحدٍ شيءٌ كان يصله.
         */
        foreach ($pending as $o) {
            $web = $o->channel === SalesChannel::WEBSITE;

            $add('order-'.$o->number, [
                'text' => $web
                    ? __('طلبٌ جديد من الموقع الإلكتروني: :number', ['number' => $o->number])
                    : __('طلب :number بانتظار التجهيز', ['number' => $o->number]),
                'time' => optional($o->ordered_at)->format('Y-m-d H:i'),
                'section' => 'orders',
                'action' => $web ? Permissions::ORDER_WEBSITE_NOTIFY : null,
                'icon' => $web ? 'globe' : 'receipt',
                'color' => $web ? 'success' : 'info',
    ...
    /**
     * كم صفًّا يعرض الجرس — وهو ما يقصّه `Topbar` بنفسه (`slice(0, 6)`).
     *
     * ورقمان يقولان الشيء نفسه يفترقان يومًا، فيحرسهما
     * `TheBellIsBuiltOnceAndSaysTheTruthTest` على الشاشة نفسِها.
     */
    public const BELL_ROWS = 6;

    /**
     * أعلى عددٍ تكتبه الشارةُ رقمًا — وفوقه تكتب «9+».
     *
     * وعليه يُبنى حدُّ العدّ: عشرةٌ من كلّ مصدر.
     *
     * ═══ ولمَ عشرةٌ بالضبط ═══
     *
     * كان الحدُّ ستًّا، فيُعدّ من كلّ مصدرٍ ستٌّ لا أكثر. ومتجرٌ فيه ثمانيةُ
     * أصنافٍ نفدت من مصدرٍ واحد كانت شارتُه تقول «٦» وعنده ثمانية — رقمٌ
     * يُقرأ دقيقًا وهو ناقص.
     *
     * وبعشرةٍ يصدق الرقمُ في كلّ حال: إن كانت الجملةُ تسعًا فما دون، فكلُّ
     * مصدرٍ دونها فيُعدّ كاملًا؛ وإن بلغت عشرًا فصاعدًا بلغ العدُّ عشرًا
     * فكتبت الشارةُ «9+» — وهو الصدق. والثمنُ ثلاثةُ أجزاءٍ من الألف من
     * الثانية، مقيسةً على الإنتاج.
     *
     * والبناءُ الكامل (١٠٠) كان يكلّف ثمانيةً وثلاثين جزءًا في كلّ صفحة
     * ليقول ما تقوله العشرةُ نفسُها للقارئ.
     */
    public const BELL_COUNT_CEILING = 9;

    /**
     * تغذيةُ الجرس — الصفوفُ والعدّاد من بناءٍ واحد.
     *
     * ═══ العطبُ الذي تعالجه ═══
     *
     * كان كلُّ موضعٍ يسأل سؤالين: `notifications()` للصفوف،
     * و`notificationsCount()` للعدّاد — وهذه تنادي تلك من جديد. فتُبنى
     * التغذيةُ **مرّتين في كلّ طلب**: واحدٌ وعشرون استعلامًا زائدةً وثمانٍ
     * وأربعون جزءًا من الثانية، في كلّ صفحةٍ يفتحها كلُّ مستخدم — وفي كلّ
     * نبضةٍ يستطلع بها الجرسُ نفسُه.
     *
     * وليس فيها ذاكرةٌ مؤقّتة: مخزنٌ ساكنٌ يبقى بين طلبٍ وطلب في الاختبارات
     * والطابور، فيُخفى تنبيهٌ ثمّ يُقرأ من نسخةٍ قديمة. فالبناءُ مرّةٌ في
     * موضع السؤال لا حفظٌ يُنسى إبطالُه.
     *
     * ═══ والصفوفُ ستٌّ لا عشرون ═══
     *
     * `buildNotifications(6)` تأخذ ستًّا **من كلّ مصدر** — والمصادرُ اثنا
     * عشر، فيخرج عشرون صفًّا أو أكثر. والجرسُ يعرض ستًّا (`slice(0, 6)`)
     * ويرمي الباقي، وقد عبَرت الشبكةَ في كلّ صفحة.
     *
     * والعدّادُ يبقى على الجملة لا على المعروض: شارةٌ تقول «٦» وتحتها
     * مئتان تُطمئن كذبًا. فيُقصّ المعروضُ ويُعدُّ الكلّ.
     *
     * @return array{items: list<array<string, mixed>>, count: int}
     */
    public static function notificationFeed(): array
    {
