...
final class InvoiceBranding
{
     ...
     *
     * ═══ والقديمُ يُمحى من القرص — وهذا ما كان ينقص ═══
     *
     * كان العمودُ وحدَه يتبدّل والملفُّ يبقى. واثنتان تترتّبان عليه:
     *
     *  • **استبدالٌ يتراكم.** من بدّل شعارَه عشر مرّاتٍ ترك عشرًا على
     *    القرص، لا يشير إليها شيء ولا يعرف بها أحد.
     *
     *  • **وحذفٌ لا يحذف.** يضغط «إزالة الشعار» فيقرأ «حُذف الشعار» —
     *    والصورةُ باقيةٌ على القرص **العامّ**، تُخدَم برابطها لمن يعرفه.
     *    وشعارُ متجرٍ ليس سرًّا، لكنّ «حُذف» كلمةٌ تعني ما تقول.
     *
     * وأخوه `Document\Branding::storeCover` يمحو القديمَ منذ كُتب — وهما
     * العمليّة نفسُها في القسم نفسِه، فافترقا في نصفها.
     *
     * والمحوُ **بعد** الحفظ: لو سقطت الكتابةُ بقي الصفُّ يشير إلى ملفٍّ
     * قائم، والعكسُ يترك عمودًا يَعِد بصورةٍ لا وجود لها. وسقوطُ المحو لا
     * يُسقط الحفظ: الإعدادُ كُتب، والقرصُ يُنظَّف يومًا.
     */
    public static function storeLogo(Business $business, ?UploadedFile $file, bool $remove = false): bool
    {
        // العمودُ الخام لا `logo` المقروءة: الأخيرةُ accessor يردّ رابطًا للعرض
        $old = $business->getRawOriginal('logo');
        ...
        $now = $business->getRawOriginal('logo');

        if (filled($old) && $old !== $now) {
            try {
                Storage::disk('public')->delete($old);
            } catch (\Throwable) {
                /* ملفٌّ لا يُمحى لا يُسقط الحفظ — كما في `Document\Branding::storeCover` */
            }
        }
