...
class Ledger
{
    ...
    public const DEFAULT_CHART = [
        ['1', 'الأصول', 'أصل', 'debit', null, [
            ...
            /*
             * ضريبةُ ما اشتُري — أصلٌ يُستردّ، لا خصمٌ مستحقّ.
             *
             * كانت ضريبةُ المشتريات تُقيَّد مدينةً في «ضريبة مستحقّة» (2300)
             * نفسِها التي تُقيَّد فيها ضريبةُ المبيعات دائنة. والصافي صحيحٌ
             * محاسبيًّا — هو ما يُدفع للجهاز فعلًا — لكنّ الحساب الواحد لا
             * يقول منه شيئًا: من فتح الميزانية قرأ رقمًا لا يعرف أهو ما
             * حصّله من زبائنه أم ما دفعه لمورّديه، ولا يطابق سطرًا واحدًا في
             * الإقرار الذي يقسمهما.
             *
             * وهما في طبيعتهما شيئان لا شيء: المخرجاتُ مالُ الجهاز في يدك،
             * والمدخلاتُ مالُك عند الجهاز. خصمٌ وأصل — لا طرفا حسابٍ واحد.
             *
             * والإقرارُ نفسه لم يكن يقرأ الحساب أصلًا: `Vat` تبنيه من
             * الطلبات وسندات المورّدين. فالفصلُ لا يغيّر رقمًا يُقدَّم، يغيّر
             * ما تراه الميزانية.
             */
            ['1250', 'ضريبة المدخلات', 'أصل', 'debit', 'tax_input'],
            ['1300', 'ذمم العملاء', 'أصل', 'debit', 'receivable'],
            /*
             * شيكاتٌ قُبضت ولم تُصرَف بعد — أصلٌ حقيقيّ وليس نقدًا.
             *
             * الذمّةُ انتقلت من العميل إلى الورقة، والورقةُ لم تصل البنك.
             * ولو رُحّلت إلى «البنك» لَقال الدفترُ إنّ المال هناك — وقد يكون
             * الشيك بتاريخٍ بعد شهرين، وقد يرتدّ.
             */
            ['1350', 'شيكات تحت التحصيل', 'أصل', 'debit', 'cheques_receivable'],
        ...
        ['2', 'الخصوم', 'خصم', 'credit', null, [
            ...
            /*
             * بضاعةٌ على الرفّ لم تصل فاتورتُها بعد — خصمٌ حقيقيٌّ لا وهميّ.
             *
             * ═══ العطبُ الذي أنشأه ═══
             *
             * كان المخزونُ يُقيَّد مدينًا عند **سند المورّد** وحده، والبضاعةُ
             * تدخل الرفَّ عند **إذن الاستلام**. فما استُلم ولم تصل فاتورتُه
             * يجلس على الرفّ ولا يعرفه الدفتر، ثمّ يُباع فيُنقص المخزونَ
             * بتكلفته. وقيس على الإنتاج: رصيدُ المخزون سالبٌ — وأصلٌ برصيدٍ
             * دائن مستحيلٌ محاسبيًّا.
             *
             * ═══ ولمَ حسابٌ وسيط ولم يُقيَّد إلى «ذمم الموردين» مباشرةً ═══
             *
             * الذمّةُ لا تنشأ بالاستلام بل بالفاتورة: مبلغُها ليس معلومًا قبلها
             * (سعرٌ يختلف، خصمٌ، ضريبةٌ)، ولا يُطالب به مورّدٌ لم يُفوتر.
             * وتقريرُ أعمار الذمم يقرأ 2100 — فحشوُه بتقديراتٍ يجعل التاجر
             * يظنّ عليه ما ليس عليه.
             *
             * فيجلس المبلغُ هنا حتى يصل السند، فيُنقل إلى الذمّة بقيمتها
             * الحقيقيّة — انظر `SupplierInvoices::approve`.
             */
            ['2150', 'بضاعة مستلمة بلا فاتورة', 'خصم', 'credit', 'goods_received_not_invoiced'],
        ]],
        ['3', 'حقوق الملكية', 'حقوق ملكية', 'credit', null, [
            ...
            /*
             * ما كان في الحساب قبل أن يعرفه النظام — ويُقيَّد هنا لا يُترك خارجًا.
             *
             * كان الرصيدُ الافتتاحيّ للحساب البنكيّ يُجمع على رصيد ورقته في
             * الشاشة ولا يدخل الدفتر أبدًا: «الحسابات البنكية» تقول ٦٠٥
             * والميزانيةُ تقول ١٠٥، ولا شيء يقول لماذا. شاشتان ترسمان رصيدَ
             * البنك برقمين — والتاجرُ يقارنهما.
             *
             * والسببُ الذي مُنع لأجله صحيح: لو قُيّد مقابلَ إيرادٍ لظهر ربحًا
             * لم يُكسب في أوّل شهرٍ يُقرأ. لكنّ الإيراد ليس مقابلَه: المالُ
             * الذي كان في البنك قبل أوّل يومٍ ليس دخلَ هذا الشهر، هو حقُّ
             * صاحبه في متجره. فيُقابَل بحقوق الملكية — وقائمةُ الدخل لا تمسّه.
             */
            ['3150', 'أرصدة افتتاحيّة', 'حقوق ملكية', 'credit', 'opening_balance_equity'],
            /*
        ...
        ['4', 'الإيرادات', 'إيراد', 'credit', null, [
            ...
            /*
             * ═══ ومبيعاتُ الموقع ورقةٌ على حدة — لا فرعٌ تحت «إيراد المبيعات» ═══
             *
             * وهي أختُه لا ابنُه، والسببُ أنّ الدفترَ لا يُعاد كتابتُه:
             * مبيعاتُ كلّ متجرٍ في أبعاد مرحَّلةٌ إلى 4100 منذ أوّل يوم. ولو
             * صار أبًا لَامتنع الترحيلُ إليه (`Account::isPostable` تمنع
             * الترحيل إلى حسابٍ له فروع) — فتسقط كلُّ بيعةِ صندوقٍ بعد
             * الترقية برسالة «لا يُرحَّل إلى…». ونقلُ سطوره إلى ابنٍ جديد
             * تزويرٌ لقيودٍ مُرحَّلة.
             *
             * والأبُ واحد — «الإيرادات» — فقائمةُ الدخل تجمعهما كما كانت،
             * والمجموعُ لا يتبدّل بحرف.
             *
             * ═══ وما قبل هذه النسخة يبقى حيث هو ═══
             *
             * بيعاتُ الموقع المُرحَّلة قبل اليوم في 4100، ولا تُنقل. ومن أراد
             * تفصيلَها لكلّ التاريخ يقرؤه من «ملخّص المبيعات» بمُرشِّح
             * القناة: ذاك يقرأ `orders.channel` — وهو مكتوبٌ على كلّ طلبٍ
             * منذ وُجد العمود.
             */
            ['4150', 'مبيعات الموقع الإلكتروني', 'إيراد', 'credit', 'sales_website'],
