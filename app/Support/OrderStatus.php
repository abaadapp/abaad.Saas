...
class OrderStatus
{
    ...
    /**
     * ما بلغ يدَ الزبون — الحالاتُ التي وقعت فيها التجربةُ فعلًا.
     *
     * ═══ ولمَ ليست `CLOSED` ═══
     *
     * `CLOSED` تضمّ **الملغيَ** معها: هي «ما انتهى أمرُه» لا «ما وصل». وطلبٌ
     * أُلغي انتهى أمرُه ولم تقع فيه تجربةٌ تُقيَّم — فسؤالُ صاحبه «كيف كانت
     * تجربتك؟» سؤالٌ عمّا لم يكن.
     *
     * تقرؤها ثلاثةُ مواضع: زرُّ تقييم Google، وحالُه في الشاشة، ودعوةُ
     * التقييم الداخليّ. وكانت القائمةُ مكتوبةً بيدٍ في كلٍّ منها — فيُضاف
     * حالٌ رابعٌ يومًا فيُضاف في اثنين ويُنسى الثالث.
     *
     * @var list<string>
     */
    public const FULFILLED = [
        self::DELIVERED,
        self::PICKED_UP,
        self::COMPLETED,
    ];

    /** أبلغ هذا الطلبُ يدَ الزبون؟ */
    public static function fulfilled(?string $status): bool
    {
        return $status !== null && in_array($status, self::FULFILLED, true);
    }
