...
class Permissions
{
    ...
    /**
     * تصديرُ بيانات النشاط — تنزيلُ أرشيفِ شهرٍ كاملًا في ملفّ.
     *
     * ═══ ولمَ فعلٌ بذاته، وقسمُ «الإعدادات» موجود ═══
     *
     * من مُنح الإعدادات مُنحها ليضبط ضريبةً أو يضيف فرعًا. وتنزيلُ الأرشيف
     * شيءٌ آخر تمامًا: ملفٌّ واحد فيه مبيعاتُ الشهر كلِّها، وأسعارُ الشراء،
     * وأسماءُ العملاء وأرقامُهم، وميزانُ المراجعة — يخرج من النظام إلى جهازٍ
     * لا سلطانَ لنا عليه. وهو أكبرُ خروجٍ للبيانات في النظام كلِّه.
     *
     * والاسمُ فيه نقطة لأنّ `isAction` تفصل الفعلَ عن القسم بها — واسمٌ بلا
     * نقطةٍ يُقرأ قسمًا فيُفتح لمن يملك «الإعدادات» وحدَها، وهو عينُ ما
     * نمنعه.
     */
    public const BUSINESS_EXPORT = 'business.export';
    ...
    /**
     * ضبطُ الموقع — تصميمُه وصفحاتُه ونطاقُه، لا تشغيلُه اليوميّ.
     *
     * وقسمُ `website` كان يجمع الاثنين: من مُنح «الموقع» ليتفقّد منتجاتِه
     * الظاهرة كلَّ صباح مُنح معه تبديلَ قالب الموقع وحذفَ صفحاته وتحويلَ
     * نطاقه. وهما عملان لا يفعلهما الشخص نفسه: الأوّل يوميٌّ يفعله الموظّف،
     * والثاني يُضبط مرّةً ويملكه صاحب المتجر.
     *
     * فالقسمُ يبقى للتشغيل (لوحةُ الموقع ومنتجاتُه)، وهذا الفعلُ يحرس الضبط.
     * ويرثه `admin` و`manager` كما كانا — انظر `ACTION_ROLES` — فلا يفقد
     * أحدٌ اليومَ بابًا كان يفتحه أمس.
     */
    public const WEBSITE_CONFIGURE = 'website.configure';

    /**
     * من يبلغه جرسُ طلبات الموقع.
     *
     * وليست فتحًا لشاشة: «المبيعات» هي التي تفتح الطلبات، وهذه تقرّر من
     * يُوقَظ لها. والموقعُ يبيع في ساعةٍ لا أحدَ فيها في المحلّ، فالطلبُ
     * ينتظر حتّى يفتح أحدٌ الشاشةَ صدفة — وباقةُ زهرٍ لموعدٍ بعد ساعتين
     * تفوت موعدَها بلا أن يُخطئ أحد.
     *
     * وتُمنح بالاسم لا تُورَث بالقسم: متجرٌ فيه عشرةٌ يفتحون «المبيعات»
     * لا يريد عشرةَ أجراسٍ تدقّ لطلبٍ واحد — يريد من يجهّزه.
     */
    public const ORDER_WEBSITE_NOTIFY = 'order.websiteNotify';
    ...
    public const ACTIONS = [
        ...
        /*
         * وتجاوزُ حظر البيع فعلٌ آخر: الحظرُ يضعه صاحبُ المتجر على زبونٍ
         * بعينه، وتجاوزُه مرّةً قرارُ من يُؤتمن عليه — لا كلِّ من يفتح
         * «العملاء». ومقبضُ الإعدادات يُطفئه كلَّه (CustomerFlags).
         */
        \App\Support\CustomerFlags::OVERRIDE => 'تجاوز حظر البيع على عميل',
        /*
         ...
         */
        /*
         * وتصديرُ بيانات النشاط: أكبرُ خروجٍ للبيانات في النظام.
         *
         * ملفٌّ واحد فيه مبيعاتُ الشهر وأسعارُ الشراء وأسماءُ العملاء
         * وأرقامُهم والميزان — يخرج إلى جهازٍ لا سلطانَ لنا عليه.
         */
        self::BUSINESS_EXPORT => 'تصدير بيانات النشاط',
    ...
    public const ACTION_ROLES = [
        ...
        /*
         * وضبطُ الموقع لصاحبه ومديره — والموظّفُ يُمنحه باسمه إن أُريد.
         *
         * والوراثةُ هنا تحفظ ما كان: هذان الدوران يفتحان شاشاتِ الموقع اليوم
         * بقسم `website`، فلا يُقفل عليهما بابٌ بإضافة فعل.
         */
        self::WEBSITE_CONFIGURE => ['admin', 'manager'],
        /*
         * وجرسُ الموقع لمن يجهّز ويوصّل — ولصاحبه ومديره.
         *
         * والوراثةُ تحفظ ما كان: هؤلاء يفتحون «المبيعات» اليوم فيصلهم صفُّ
         * «بانتظار التجهيز» لكلّ طلبٍ جديد — فلا يُقطع عنهم ما يصلهم بإضافة
         * فعل. والمحاسبُ يخرج منها بقصد: تجهيزُ باقةٍ ليس عمله، وهو يقرأ
         * البيعةَ في دفترها حين تقع.
         */
        self::ORDER_WEBSITE_NOTIFY => ['admin', 'manager', 'sales', 'delivery'],
         ...
         *
         * ولم يُورَّث للمحاسب بدوره وإن كان أقربَ الناس إليه عملًا: من يقرأ
         * الدفترَ في الشاشة يُؤتمن على قراءته، ولا يعني ذلك أنّه يُؤتمن على
         * إخراج الشهر كلِّه في ملفٍّ يحمله معه يوم يترك العمل. فيُمنح بالاسم
         * لمن يُراد — وهي ضغطةٌ واحدة في «صلاحيات الموظفين».
         */
        self::BUSINESS_EXPORT => ['admin', 'manager'],
        /*
    ...
    public static function beyond(?User $actor, User $employee): array
    {
         ...
         */
        if (self::isOwner($actor) || $actor->id === $employee->id) {
            return [];
        }
    ...
    /**
     * أصاحبُ النشاطِ هذا؟ — سؤالٌ واحدٌ يُسأل في ثلاثة أبواب.
     *
     * ═══ والدورُ وحده لا يجيبه ═══
     *
     * صفٌّ دورُه `admin` وصلاحياتُه مخصّصةٌ ليس صاحبَ النشاط: هو شريكٌ
     * ضُيّق عمدًا، و`allows`/`may` تقرآن قائمتَه لا دورَه. فمن أعفاه بالدور
     * أعطاه على **الحسابات** ما نُزع عنه في **الشاشات**.
     *
     * ═══ وثلاثةُ أبوابٍ كانت تسأله بثلاث صيغ ═══
     *
     * `beyond` وحدَها كانت تسأله تامًّا. وبابا `EmployeeController` —
     * `refuseGrantingMoreThanIHave` و`refuseRaisingMyself` — كانا يسألان
     * «أدورُه admin؟» فحسب. وثمنُه أنّ الشريكَ المُضيَّق:
     *
     *   ١ — يُنشئ حسابًا بوظيفةٍ دورُها «مدير فرع» — وهي `'*'` — بكلمة مرورٍ
     *       يكتبها بيده، فيدخل بها فيملك ما لا يملكه صاحبُ الحساب الذي هو
     *       فيه. قِسناه: ٣٠٢ وصفٌّ دورُه `manager`.
     *   ٢ — ويبدّل **وظيفةَ نفسِه** إلى تلك الوظيفة، فيرتفع في صفّه هو.
     *       قِسناه: ٣٠٢ ودورُه صار `manager`.
     *
     * وفي كلتيهما كانت الشاشةُ تقول الصوابَ وحدها: `blockedTitles` تُبنى من
     * `mayAssignRole` فتُعطّل الوظيفةَ في القائمة — ويقبلها الخادم. وفحصان
     * لسؤالٍ واحد يفترقان يوم يُبدَّل أحدهما، وقد افترقا.
     */
    public static function isOwner(?User $actor): bool
    {
    ...
    public const ALIASES = [
         ...
         *
         * ولولا هذا السطرُ لاستُنتج له قسمٌ اسمُه `archives` لا وجودَ له في
         * `SECTIONS` — فيردّ البابُ كلَّ من صلاحيّاتُه مخصَّصةٌ يدويًّا، ولو
         * مُنح `business.export` بالاسم. ويمرّ صاحبُ النشاط وحده لأنّ له
         * `'*'` — فيبدو البابُ عاملًا وهو مغلقٌ على من قُصد أن يُفتح له.
         *
         * والفعلُ فوق القسم لا بدلًا منه: `may:business.export` على المسار.
         */
        'archives' => 'settings',
