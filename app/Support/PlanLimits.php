...
class PlanLimits
{
    ...
    public static function used(int $businessId, string $key): int
    {
        return match ($key) {
             ...
             *
             * لا مسارَ حذفٍ للموظّف في النظام — والصحيح ألّا يكون: `orders.user_id`
             * و`shifts` و`payroll_lines` وسجلُّ النشاط كلُّها تشير إليه، ومحوُه
             * يمحو نسبةَ ما باع إليه. فالتعطيلُ هو ما يفعله التاجرُ حين يترك
             * موظّفٌ العمل، وهو المقبض الوحيد الذي أُعطي له.
             *
             * وكان العدُّ يشمله. فمتجرٌ بلغ سقفَه يعطّل من تركه ثمّ يوظّف بديلًا
             * فيُردّ: «بلغت حدّ باقتك: ٣ من الموظفين» — وهو ينظر إلى اثنين
             * يعملان. ولا مخرجَ له إلّا ترقيةُ الباقة، أو أن يُعيد استعمال
             * حساب الراحل باسمٍ آخر — فتُنسب مبيعاتُ الأوّل إلى الثاني.
             *
             * والباقةُ تبيع موظّفين يعملون، لا صفوفًا في جدول. فيُعدّ النشطون.
             *
             * وإعادةُ التفعيل تُقاس بالسقف كذلك — وإلّا كان التعطيلُ بابًا
             * يُلتفّ به عليه: عطِّل، وظِّف، ثمّ أعِد التفعيل.
             * انظر `EmployeeController::toggleStatus`.
             */
            'employees' => User::where('business_id', $businessId)->where('status', 'نشط')->count(),
