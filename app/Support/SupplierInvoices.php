...
final class SupplierInvoices
{
    ...
    public static function approve(SupplierInvoice $invoice, ?User $by = null, ?string $overrideReason = null): SupplierInvoice
    {
        return DB::transaction(function () use ($invoice, $by, $overrideReason) {
            ...
            /*
             * الضريبة تدخل في التكلفة لا في حسابٍ مستقلّ — كما كانت.
             *
             * فصلُها يصحّ لمن كان مسجَّلًا في الضريبة فيستردّها؛ ومن لم يكن
             * فالضريبة عنده جزءٌ من ثمن البضاعة، وفصلُها يُنشئ أصلًا لا
             * يُستردّ أبدًا ويُنقص تكلفة المخزون.
             */
            Ledger::post(
                $locked->business_id,
                __('سند مورّد: ').$locked->supplier_ref,
                [
                    ['account' => 'inventory', 'debit' => $total, 'memo' => $locked->supplier?->name],
