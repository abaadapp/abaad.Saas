...
final class Support
{
    ...
    /**
     * محادثاتُ المتجر التي يراها هذا القارئ.
     *
     * ═══ العطبُ الذي يعالجه ═══
     *
     * كانت المحادثةُ للمتجر لا لمن فتحها: كلُّ موظّفٍ يدخل اللوحة — والكاشير
     * منهم، فمركزُ المساعدة من هيكل اللوحة لا يُحرَس بقسم — يقرأ ما كتبه
     * صاحبُ المتجر لأبعاد، وما ردّت به أبعاد عليه، ويُنبَّه في جرسه «لديك
     * ردّ جديد من دعم أبعاد» عن محادثةٍ ليست له. وصاحبُ المتجر يشكو
     * اشتراكَه أو مبيعاتِه أو موظّفًا بعينه — إلى أبعاد، لا إلى الكاشير.
     *
     * فالقاعدة: **من فتح المحادثة يراها، وصاحبُ المتجر يرى محادثاتِ متجره
     * كلَّها**. والموظّفُ — مديرًا كان أو كاشيرًا — يرى ما فتحه هو وحده.
     * ومحادثةٌ بلا فاتح (قناةٌ خارجيّة) لصاحب المتجر وحده.
     *
     * والاستعلامُ واحدٌ يقرؤه القائمةُ والمحادثةُ والمرفقُ والشارةُ والجرس —
     * فلا يُغلَق بابٌ ويبقى الشبّاكُ مفتوحًا.
     */
    public static function visibleTo(User $user)
    {
        return SupportConversation::where('business_id', $user->business_id)
            ->when($user->role !== 'admin', fn ($q) => $q->where('opened_by', $user->id));
    }
    ...
    public static function businessBadge(User $user): int
    {
        ...
        /* وما لا يراه لا يُعدّ له — انظر `visibleTo` */
