...
class TenantTables
{
    ...
    public const ORDER = [
        ...
        /*
         * وسجلُّ نشرِ الواجهة الخاصّة بعد النسخ: `store_sites.published_version_id`
         * يشير إلى `website_versions` — فيُدرَج بعدها ولا حلقةَ بينهما.
         *
         * ونسخُه لازمٌ لا زينة: فيه **مسوّدةُ** التاجر — ما كتبه ولم ينشره
         * بعد. ومتجرٌ يُستعاد بلا مسوّدته يفقد عملَ يومٍ لم يبلغ زبونَه،
         * ويعود محرّرُه يعرض المنشورَ كأنّ صاحبَه لم يكتب شيئًا.
         */
        'store_sites',
        'whatsapp_messages',
        /*
         * ودفترُ الوارد يُنسخ مع المتجر — لا لأنّ فيه رسائل، بل لأنّ فيه
         * ما يمنع تكرارَها.
         *
         * ليس فيه نصُّ رسالةٍ ولا مرفق (انظر الترحيل): فيه معرّفُ الرسالة
         * عند ميتا وختمُ الردّ عليها. ولو تُرك خارج النسخة لَعاد المتجرُ
         * بعد الاستعادة يردّ آليًّا على رسائلَ ردّ عليها أمس — ولَقُبلت
         * إشعاراتٌ مكرّرةٌ كأنّها جديدة.
         */
    ...
    public const NOT_MINE = [
        ...
        /*
         * وحزمةُ الرسائل مثلُهما: شراءٌ من أبعاد لا بيانةٌ من دفتر المتجر.
         *
         * واستعادتُها أسوأ من استعادة فاتورة: صفٌّ `مصدَّرة` يعود من ملفٍّ
         * يرفعه التاجر يصير طلبًا ينتظر سدادًا لا أحد يذكره — أو `مطلوبة`
         * تُعتمَد مرّةً ثانية فتُصدَر فاتورةٌ ثانية لحزمةٍ اشتُريت. والرصيدُ
         * نفسُه عمودٌ في `businesses` يُحدَّث بحقولٍ آمنة، فلا يضيع.
         */
        'whatsapp_message_packs' => 'حزمُ رسائلَ يشتريها المتجر من أبعاد — سجلُّ شراءٍ عند المنصّة لا بياناتُ متجر',
        ...
        /*
         * ومتابعةُ التنبيهات مثلُها — ولا يُفقد باستبعادها شيء.
         *
         * الصفُّ يقول «أجّلتُ هذا» أو «أنجزتُه»، والتنبيهُ نفسُه **مشتقٌّ من
         * البيانات الحيّة** لا مخزَّن: يُبنى من الصنف الناقص ومن الورقة
         * المعلَّقة ومن الطلب الذي ينتظر. فاستعادةُ القاعدة تُعيد الأسبابَ
         * كلَّها، ويُبنى الجرسُ من جديدٍ صادقًا.
         *
         * واستعادتُها أضرُّ من تركها: «أُنجز» قديمٌ يعود فوق مشكلةٍ حيّة
         * فيكتمها، و«مؤجَّلٌ إلى» بتاريخٍ مضى يُقرأ منقضيًا أو باقيًا بحسب
         * متى استُعيد الملفّ — سلوكٌ يتبدّل بساعة الاستعادة لا بحال المتجر.
         */
        'notification_states' => 'متابعةُ مستخدمٍ لتنبيهاته — حالةُ عرضٍ تُشتقّ من البيانات، لا بيانات',
        /*
