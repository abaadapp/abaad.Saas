...
class Preview
{
    ...
    public static function resolve(array $snapshot, int $businessId): array
    {
        ...
        /*
         * ═══ ومفاتيحُ المتجر معها: «قبول الطلبات» و«إظهار الأسعار» ═══
         *
         * كانت تُجمَّد في اللقطة وحدَها بين جيرانها. فيُطفئ التاجر «قبول
         * الطلبات» — لأنّه مسافر، أو نفدت بضاعتُه، أو يراجع أسعاره — ويقرأ
         * «حُفظت إعدادات المتجر» خضراءَ، **وموقعُه يبقى يستقبل الطلبات**.
         *
         * وأسوأُ منه أنّ **معاينتَه تُطيعه**: قِيس الفرقُ فكان
         * `المعاينة = false` و`المنشور = true`. فيفتح معاينته فلا يرى زرَّ
         * الطلب، ويطمئنّ — وزبونُه يراه ويطلب. وهو العطبُ الذي وُضع له
         * `RendererParityTest` أصلًا: أن يرى التاجر في معاينته غير ما يرى
         * زبونُه.
         *
         * وهي حالُ النشاط الآن لا تصميمٌ يُجمَّد — كالشعار والعملة سواء.
         * ومن يُطفئ استقبالَ الطلبات لا يخطر له أن عليه أن ينشر موقعه.
         *
         * والوجهةُ تُقرأ من **اللقطة** لا من المسوّدة: من بدّل وجهة موقعه
         * إلى «تعريفيّ» ولم ينشر، موقعُه المنشور ما زال متجرًا — فتُقاس
         * مفاتيحُه بوجهته المنشورة.
         */
        $snapshot['commerce'] = Publication::commerceFor(
            (string) ($snapshot['goal'] ?? Blueprints::STORE),
            $businessId,
        );
