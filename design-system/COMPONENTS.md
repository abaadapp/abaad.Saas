...
بابٌ قبل الشاشة حين لا يعمل شيءٌ ممّا وراءه: أيقونةٌ واسمٌ وسطرٌ و**فعلٌ واحد**.

```tsx
<Gate icon={MessageCircle} title="واتساب غير مربوط بعد" description="…"
      tint="#25d366" action={<Button size="lg" asChild><Link …/></Button>} note={…} />
```
**Props:** `icon` · `title` · `description?` · `tint?` · `action?` · `note?`
و`ConnectGate` في `Connect.tsx` هي هذه مع زرِّ «ربط مع أبعاد» — فلا تُرسم بابًا بيدك.
