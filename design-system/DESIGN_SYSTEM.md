...
وحدها، وتُشتقّ كلُّها من لونٍ واحدٍ يختاره التاجر (`Document\Theme`).

### تشريحُ الورقة

```
▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀ .brandrule — ٢٫٥pt بلون التاجر ▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀

  [شعار]                              ┌──────────────────────┐
  اسمُ المتجر (.shop)                  │ .doctype  أمر شراء   │
  نوعُه — مدينتُه                       │ .docnum   PU-000123  │  ← td.idpanel
                                       │ ▢ .stamp  مُرسل ▢     │
                                       └──────────────────────┘
──────────────────────────── .headrule ────────────────────────────

┃ td.party ┃ td.gap ┃ td.party          ← الأطرافُ متقابلةً، أرضيّةٌ وحافّة
┌──────────┬──────────┬──────────┐     ← table.metastrip (أربعةٌ في السطر)
▓▓▓▓▓▓▓ table.items th ▓▓▓▓▓▓▓▓▓▓      ← رأسٌ مصمتٌ ونصٌّ معكوس
  tr / tr.alt …                        ← تناوبٌ محسوبٌ في القالب
                       ┌ td.totalcard ┐
                       │ ▓ tr.grand ▓ │  ← كتلةٌ مصمتة، أقوى ما في الورقة
┃ .panel — ملاحظاتٌ وشروط
[ td.box ] [ td.box ]  ← خاناتُ توقيعٍ منقوطة        [ .qr-block ]
─────────────────── table.docfoot ───────────────────
الرقم الضريبي: OM…                                        PU-000123
```

ورقمُ الصفحة «٢ / ٣» ليس في الجسد: يرسمه `MpdfDriver::frame` في تذييل **كلّ**
صفحة، ومعه سياقُ المستند الذي يمرّره `DocumentRenderer::pdf`.

### أربعُ قواعدَ يفرضها المحرّك (قِيست، لا تُفترض)

| القاعدة | ما يقع بدونها |
|---|---|
| **كلُّ قيمةٍ مرّتين**: `var(--x)` ثمّ القيمةُ الحرفيّة | mpdf يتجاهل المتغيّرات صامتًا — ورقةٌ بلا لون |
| **الأرضيّةُ على الخليّة** (`td.party`) لا على `div` داخلها | mpdf يُقلّص كتلَ `div` داخل الخلايا إلى عرض نصّها — ولو صُرّح `width:100%` |
| **أرضيّةُ الإجمالي على `tr`** أيضًا لا على خليّتيه | خيطٌ أبيضُ رفيع يشقّ الصندوق نصفين |
| **التناوبُ محسوبٌ في القالب** لا بـ`:nth-child` | مخطّطٌ في المتصفّح، مسطّحٌ في الـPDF |

وثلاثتُها الأخيرةُ **لا تظهر في المتصفّح إطلاقًا**. فمعاينةٌ سليمةٌ ليست دليلًا:
القياسُ على الملفّ المطبوع نفسِه — ويحرسها `TheDocumentIsDesignedNotGeneratedTest`.
