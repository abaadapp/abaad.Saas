...
```
▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔ .brandrule — ٢pt بلون التاجر ▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔▔

  [شعار]                                        أمر شراء
  اسمُ المتجر (.shop)                     ← .doctype، ٢٨pt بحبر التاجر
  نوعُه — مدينتُه                            رقم المستند
                                          PU-000123   (.docnum)
                                          ▢ .stamp  مُرسل ▢
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈ .headrule — ٠٫٥pt محايدة ┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈

  td.party        │  td.divide          ← عمودان وفاصلٌ شعرة، بلا بطاقات
░░░░░░░░░░ table.metastrip — أرضيّةٌ فاتحةٌ جدًّا، بلا خطوط ░░░░░░░░░░
  الصنف        الكمّيّة     السعر      الإجمالي   ← table.items th
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ← ١pt بحبر التاجر
  tr … tr …                                     ← شعرةٌ تحت كلٍّ، بلا زِبرة

  .panel — ملاحظات        ░ td.totalcard — أرضيّةٌ فاتحة ░
  [ .qr-block ]           ░ ━━━━━━━━━━━━━━━━━━━━━━━━━ ░
                          ░ tr.grand — ١٥pt بحبر التاجر ░
┈┈┈ td.box ┈┈┈   ┈┈┈ td.box ┈┈┈   ← خطٌّ يُوقَّع فوقه
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈ table.docfoot ┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
الرقم الضريبي: OM…                                        PU-000123
```

والخاتمةُ عمودان تبنيهما `partials/totals` لتسع أوراقٍ دفعةً واحدة: ما يُقال
عن الصفقة — ملاحظاتٌ ورمز — في جهة، وما يُدفع فيها في الجهة المقابلة.
...
### لونُ التاجر لمسةٌ لا مساحة

لا يملأ سطحًا ليس خطًّا. أربعةُ مواضعَ لا خامسَ لها: `.brandrule` و`.cover-plain`
(وكلاهما ≤ ٤pt ارتفاعًا)، وعنوانُ المستند، وخطُّ رأس الجدول، وخطُّ الإجمالي مع
رقمه. وما بقي من أسطحٍ على الورقة اثنان فاتحان جدًّا: `table.metastrip`
(`surface`) و`td.totalcard` (`primary_wash`، ستّةٌ بالمئة من لونه).

وكان قبل ذلك يملأ أربعةَ أسطحٍ مصمتة — رأسَ الجدول، ولوحةَ المستند، وصندوقَ
الإجمالي، وبطاقتَي الطرفين — فتُقرأ الورقةُ لوحةَ تحكّمٍ لا ورقةً تُحفظ في ملفّ،
وتشرب الطابعةُ حبرَها. ويحرس هذا `test_the_brand_colour_fills_nothing_taller_than_a_rule`.
...
| **الأرضيّةُ والحدُّ على الخليّة** (`td.totalcard`, `td.divide`) لا على `div` داخلها | mpdf يُقلّص كتلَ `div` داخل الخلايا إلى عرض نصّها — ولو صُرّح `width:100%` |
| **جدولٌ بعرض ١٠٠٪ لا يقع داخل خليّةٍ ضيّقة** | يُربك حسابَ العرض الأدنى فتنسحق الأعمدةُ إلى حرفين — ولذلك يُرسم الرمزُ في الخاتمة كتلًا متراكبة لا صفًّا |
| **لا محدّدَ بنيويًّا** (`:nth-child`) | ما يُبنى عليه يخرج في المتصفّح ولا يخرج في الـPDF |

ورابعةٌ خرجت من الاستعمال ولم تخرج من الذاكرة: **أرضيّةُ صفٍّ تُكتب على `tr`
وعلى خليّتيه معًا**، وإلّا شقّها خيطٌ أبيضُ رفيع نصفين. لا أرضيّةَ على صفٍّ اليوم
— فمن أعادها يومًا فليعدها على الاثنين.
