...
export const PRODUCT_TABS: SectionTab[] = [
     ...
     *
     * ويُحسب من المفتاح **أو من بوتيكٍ قائمٍ عنده** (`Boutiques::holds`):
     * مفتاحٌ يُطفأ لا يحبس كشفَ مالٍ ما زال يُكتب.
     */
 ...
 */
/**
 * إعداداتُ الموقع — ستُّ شاشاتٍ تُضبط مرّةً ثمّ تُترك.
 *
 * ولم تعد عائلةَ الشريط الجانبيّ: الشريطُ يفتح لوحةَ التشغيل اليوميّة، وهذه
 * تُفتح من «الإعدادات ‹ الموقع الإلكتروني». والترتيبُ ترتيبُ العمل: ما هو
 * الموقع، ثمّ كيف يبدو، ثمّ ماذا فيه، ثمّ ماذا يبيع، ثمّ أين عنوانه، ثمّ كيف
 * يُوجَد.
 *
 * ومساراتُها لم تتبدّل: روابطُ محفوظةٌ وإشاراتٌ مرجعيةٌ إليها، و404 عليها
 * فقدانٌ لا نقل.
 */
export const WEBSITE_TABS: SectionTab[] = [
    { label: 'عام', routeName: 'admin.website.site', section: 'website' },
    { label: 'التصميم', routeName: 'admin.website.design', section: 'website' },
    { label: 'الصفحات', routeName: 'admin.website.pages', section: 'website' },
    { label: 'المتجر والطلبات', routeName: 'admin.website.shop', section: 'website' },
...
/*
 * ═══ ومن لبس واجهةً خاصّة — الشريطُ نفسُه، ستّةً ═══
 *
 * والمساراتُ هي مساراتُ جاره حرفًا: `site` و`design` و`pages` و`shop`
 * و`domain` و`seo`. وإنّما تُكتب قائمةً على حدة لا `WEBSITE_TABS` نفسِها،
 * لأنّ الشاشات خلفها من صنعةٍ أخرى — فيوم يفترق تبويبٌ يفترق في موضعه
 * ولا يُشقّ الشريطُ من جديد.
 *
 * ═══ وما قِيس فيه من قبل — يُقال ولا يُخفى ═══
 *
 * جُمعت هذه الستُّ مرّةً في صفحةٍ واحدةٍ بعمودٍ يقفز، لأنّ التوزيعَ قِيس
 * فوُجد غيرَ عادل: «الدومين» ثلاثةُ مقابض و«الظهور في البحث» ثلاثة
 * و«الصفحات» ثلاثة — و«المتجر والطلبات» اثنان وثلاثون.
 *
 * وعادت ستًّا بقرارٍ من صاحب المنتج: أن يكون لصاحب الواجهة الخاصّة ما
 * لجاره حرفًا بحرف، لا شكلٌ ثانٍ يتعلّمه وحدَه. والعطبُ المقيسُ لم يُهمَل:
 * «المتجر والطلبات» تُبنى مجموعاتٍ **بزرّ حفظٍ واحد**، لا اثنين متجاورين
 * لا يحفظ أحدُهما ما يحفظه الآخر.
 *
 * و«التصميم» يقود إلى محرّر صفحته: لا قوالبَ لواجهةٍ خاصّة تُبدَّل
 * (`store_theme` ميّتٌ — انظر `Store\PageEditor::DEAD`)، فتصميمُها ترتيبُ
 * أقسامها. ولا تُخترع له لوحةُ ألوانٍ لا يقرؤها قالبُه.
 */
export const THEME_TABS: SectionTab[] = [
    { label: 'عام', routeName: 'admin.website.site', section: 'website' },
    { label: 'التصميم', routeName: 'admin.website.design', section: 'website' },
    { label: 'الصفحات', routeName: 'admin.website.pages', section: 'website' },
    { label: 'المتجر والطلبات', routeName: 'admin.website.shop', section: 'website' },
