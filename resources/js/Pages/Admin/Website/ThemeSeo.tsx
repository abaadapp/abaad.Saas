...
/**
 * «الموقع الإلكتروني ‹ الظهور في البحث» لصاحب الواجهة الخاصّة.
 *
 * ═══ وما كان ناقصًا ═══
 *
 * عنوانُ متجره في نتيجة غوغل كان اسمَ نشاطه، ووصفُه أوّلَ سطورِ نبذته —
 * يُكتبان في `<head>` ولا يملك إليهما بابًا. وهما ما يُضغط أو لا يُضغط:
 * سطرٌ يقول «ريبون» وحدَه لا يقول لمن يبحث عن «ورد توصيل مسقط» شيئًا.
 *
 * ═══ والمعاينةُ تُرى قبل أن تُحفظ ═══
 *
 * فمن كتب عنوانًا لا يعرف أطولٌ هو أم قصير حتى يراه مقصوصًا في نتيجة.
 * والحدُّ إرشادٌ: ما تجاوزه يُحفظ ويُقصّ في غوغل، ولا يُردّ حفظُه برسالة.
 */
export default function ThemeSeo() {
    const { site, seo, fallback, limits } = usePage<PageProps<Props>>().props;
    ...
    return (
        <AdminLayout title="الموقع الإلكتروني">
            <ThemeHeader
                ...
                subtitle={t('ما يراه من يبحث عن متجرك في غوغل — قبل أن يفتحه')}
            />

            <form onSubmit={save}>
                <SettingsSection
                    title="نتيجتك في غوغل"
                    description="العنوان والوصف اللذان يظهران في نتائج البحث — اتركهما فارغين فيبقى ما يُحسب من اسمك ونبذتك."
                    divided
                >
                    {/* ═══ المعاينة ═══ */}
                    ...
                    <SettingsGroup title="الفهرسة">
                        ...
                        {/*
                            ...
                            من ظنّها مفتاحَ نشرٍ أطفأها ليُغلق متجره، وهو مفتوحٌ
                            لكلّ من يملك الرابط. والمفتاحُ الذي يُغلق في
                            «الدومين».
                        */}
                        {!form.data.store_seo_index && (
                            ...
                            >
                                {t('هذا لا يُغلق متجرك — من يملك الرابط يفتحه. الإغلاق من «الدومين ‹ نشر المتجر».')}
                            </p>
                        )}
                    </SettingsGroup>
