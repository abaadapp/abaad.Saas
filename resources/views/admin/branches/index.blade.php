<x-layouts::admin title="الفروع">
    ...
    <div x-data="{ adding: {{ $errors->any() ? 'true' : 'false' }} }">
        <x-page-header
            title="الفروع"
            subtitle="أضِف فروع نشاطك وأدرها من مكان واحد"
            :breadcrumbs="['الرئيسية' => route('admin.dashboard'), 'الفروع' => '#']"
        >
            <x-slot:actions>
                <x-button variant="primary" size="md" icon="plus" type="button" x-on:click="adding = !adding">إضافة فرع</x-button>
            </x-slot:actions>
        </x-page-header>
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400">إجمالي الفروع</p>
                    </div>
                </div>
            </div>
            ...
            <div x-show="adding" x-cloak class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <div class="flex items-center gap-2 mb-5">
                    ...
                    <h3 class="text-lg font-bold text-gray-800">إضافة فرع جديد</h3>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <h3 class="text-lg font-bold text-gray-800 mb-5">قائمة الفروع</h3>
                @if (count($branchList))
                ...
                @else
                    <x-empty-state icon="git-branch" title="لا توجد فروع" message="أضف أول فرع لنشاطك من زر «إضافة فرع» بالأعلى." />
                @endif
            </div>
        </div>
    </div>
