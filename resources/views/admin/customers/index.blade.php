<x-layouts::admin :title="__('العملاء')">
    <x-page-header
        :title="__('العملاء')"
        :subtitle="__('إدارة قاعدة عملاء المحل وسجل مشترياتهم ونقاط ولائهم')"
        :breadcrumbs="[__('الرئيسية') => route('admin.dashboard'), __('العملاء') => '#']"
    >
        <x-slot:actions>
            {{-- زر «المزيد» (تصدير/استيراد) --}}
            <x-dropdown align="left" width="w-60">
                <x-slot:trigger>
                    <span title="{{ __('المزيد') }}" aria-label="{{ __('المزيد') }}"
                ...
                <p class="px-4 pt-2 pb-1 text-[11px] font-semibold text-gray-400">{{ __('تصدير') }}</p>
                <a href="{{ route('admin.customers.export.xlsx') }}" class="flex items-center gap-2.5 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                    <x-icon name="file-spreadsheet" class="w-4 h-4 text-green-600" /> {{ __('تصدير Excel (xlsx)') }}
                </a>
                <a href="{{ route('admin.customers.export.pdf') }}" target="_blank" class="flex items-center gap-2.5 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                    <x-icon name="file-text" class="w-4 h-4 text-red-600" /> {{ __('تصدير PDF') }}
                </a>
                <a href="{{ route('admin.export.customers') }}" class="flex items-center gap-2.5 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                    <x-icon name="file-down" class="w-4 h-4 text-gray-500" /> {{ __('تصدير CSV') }}
                </a>

                <div class="my-1 border-t border-gray-100"></div>
                <p class="px-4 pt-1 pb-1 text-[11px] font-semibold text-gray-400">{{ __('استيراد') }}</p>
                <button type="button" x-on:click="$dispatch('open-modal','import-customers')" class="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                    <x-icon name="upload" class="w-4 h-4 text-primary-600" /> {{ __('استيراد من ملف…') }}
                </button>
            </x-dropdown>

            <x-button variant="primary" size="md" icon="user-plus" x-on:click="$dispatch('open-modal','add-customer')">{{ __('إضافة عميل') }}</x-button>
        </x-slot:actions>
    </x-page-header>
    ...
    {{-- جدول العملاء --}}
    <x-table :headers="[__('العميل'), __('الهاتف'), __('البريد'), __('عدد الطلبات'), __('إجمالي المشتريات'), __('آخر طلب'), __('نقاط الولاء'), __('إجراءات')]">
        @foreach ($customers as $customer)
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 text-gray-700 whitespace-nowrap">{{ __(':n طلب', ['n' => $customer['orders']]) }}</td>
                ...
                <td class="px-4 py-3 whitespace-nowrap">
                    <x-badge type="warning" :text="__(':n نقطة', ['n' => $customer['points']])" />
                </td>
                <td class="px-4 py-3 whitespace-nowrap">
                    <x-button variant="light" size="sm" icon="eye" :href="route('admin.customers.show', $customer['id'])">{{ __('عرض') }}</x-button>
                </td>
            </tr>
        @endforeach
    ...
    {{-- نافذة إضافة عميل --}}
    <x-modal name="add-customer" :title="__('إضافة عميل جديد')" maxWidth="max-w-lg">
        ...
        <x-slot:footer>
            ...
            <x-button variant="primary" size="md" type="submit" form="add-customer-form" icon="check">{{ __('حفظ العميل') }}</x-button>
        </x-slot:footer>
    </x-modal>

    {{-- نافذة استيراد العملاء --}}
    @php $importBranches = \App\Support\Demo::branches(); @endphp
    <x-modal name="import-customers" :title="__('استيراد العملاء من ملف')" maxWidth="max-w-lg">
        <form id="import-customers-form" method="POST" action="{{ route('admin.customers.import.upload') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('ملف العملاء') }}</label>
                ...
                <p class="text-xs text-gray-400 mt-1.5">{{ __('الصيغ المدعومة: CSV، XLS، XLSX، XLSM — الأعمدة: الاسم، الهاتف، البريد، العنوان، الفرع، النقاط. يمكنك تصدير ملف ثم تعديله وإعادة استيراده.') }}</p>
            </div>
            ...
            <div class="flex items-start gap-2 bg-primary-50 text-primary-700 rounded-xl px-3 py-2.5 text-xs">
                ...
                <span>{!! __('العملاء الموجودون مسبقًا (بنفس الهاتف) <strong>يُحدَّثون بدل تكرارهم</strong>، مع الحفاظ على فروعهم. وستظهر <strong>معاينة كاملة</strong> قبل التأكيد.') !!}</span>
            </div>
        </form>

        <x-slot:footer>
            <x-button variant="ghost" size="md" x-on:click="$dispatch('close-modal')">{{ __('إلغاء') }}</x-button>
            <x-button variant="primary" size="md" type="submit" form="import-customers-form" icon="eye">{{ __('معاينة الملف') }}</x-button>
        </x-slot:footer>
    </x-modal>
</x-layouts::admin>
