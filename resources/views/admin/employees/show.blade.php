...
<x-layouts::admin :title="$employee['name']">
    <x-page-header
        title="ملف الموظف"
        subtitle="عرض بيانات الموظف وأدائه وصلاحياته"
        :breadcrumbs="['الرئيسية' => route('admin.dashboard'), 'الموظفون' => route('admin.employees.index'), $employee['name'] => '#']"
    >
        <x-slot:actions>
            <x-button variant="outline" size="md" icon="arrow-right" :href="route('admin.employees.index')">رجوع</x-button>
            <x-button variant="primary" size="md" icon="pencil" :href="route('admin.employees.edit', $employee['id'])">تعديل</x-button>
        </x-slot:actions>
    </x-page-header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
                ...
                <div class="mt-5 flex flex-wrap items-center justify-center gap-2">
                    <form method="POST" action="{{ route('admin.employees.toggle', $employee['id']) }}">
                        ...
                        @if ($employee['status'] === 'نشط')
                            <x-button variant="outline" size="sm" type="submit" icon="lock">تعطيل الحساب</x-button>
                        @else
                            <x-button variant="success" size="sm" type="submit" icon="lock-open">تفعيل الحساب</x-button>
                        @endif
                    </form>
                    <form method="POST" action="{{ route('admin.employees.resetPassword', $employee['id']) }}"
                          @submit="if(!confirm('توليد كلمة مرور مؤقتة جديدة؟')) $event.preventDefault()">
                        @csrf
                        <x-button variant="light" size="sm" type="submit" icon="key-round">إعادة تعيين كلمة المرور</x-button>
                    </form>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <h3 class="text-sm font-bold text-gray-800 mb-4 flex items-center gap-2">
                    <x-icon name="briefcase" class="w-4 h-4 text-primary-600" /> معلومات الوظيفة
                </h3>
                <ul class="space-y-3 text-sm">
                    <li class="flex items-center justify-between">
                        <span class="text-gray-500">الدور</span>
                        <span class="text-gray-800">{{ $employee['role'] }}</span>
                    </li>
                    <li class="flex items-center justify-between">
                    ...
                    <li class="flex items-center justify-between">
                        <span class="text-gray-500">رقم الموظف</span>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <h3 class="text-sm font-bold text-gray-800 mb-4 flex items-center gap-2">
                    <x-icon name="target" class="w-4 h-4 text-primary-600" /> ملخص الأداء
                </h3>
                <div class="space-y-4">
                    <div>
                        <div class="flex items-center justify-between text-sm mb-1.5">
                            <span class="text-gray-500">تحقيق الهدف الشهري</span>
                    ...
                    <div>
                        <div class="flex items-center justify-between text-sm mb-1.5">
                            <span class="text-gray-500">رضا العملاء</span>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <x-stat-card label="إجمالي المبيعات" :value="\App\Support\Demo::money($employee['sales'])" icon="trending-up" color="success" />
                <x-stat-card label="عدد الطلبات" value="128" icon="shopping-bag" color="primary" />
                <x-stat-card label="متوسط الطلب" :value="\App\Support\Demo::money($employee['sales'] / 128)" icon="calculator" color="info" />
                <x-stat-card label="التقييم" value="4.7 / 5" icon="star" color="warning" />
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm" x-data="{ tab: 'sales' }">
                ...
                <div x-show="tab === 'permissions'" x-cloak class="p-6">
                    <p class="text-sm text-gray-500 mb-4">تحكم في صلاحيات هذا الموظف داخل النظام.</p>
                    @php
                        $permissions = [
                            'فتح نقطة البيع' => true,
                            'إنشاء طلب جديد' => true,
