<x-layouts::admin title="تفاصيل المنتج">
    ...
    <x-page-header title="{{ $product['name'] }}" subtitle="{{ $product['cat'] }} · {{ $product['sku'] }}"
        :breadcrumbs="['الرئيسية' => route('admin.dashboard'), 'المنتجات' => route('admin.products.index'), $product['name'] => '#']">
        <x-slot:actions>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div class="flex items-center gap-2 mb-3">
                <x-badge :text="$product['stock_status']" />
                <x-badge :type="$product['active'] ? 'success' : 'gray'">{{ $product['active'] ? 'مفعّل' : 'غير مفعّل' }}</x-badge>
                @if ($product['discount'])
                    <x-badge type="secondary">خصم {{ $product['discount'] }}%</x-badge>
                @endif
            </div>
            ...
            <div class="mt-3 flex items-baseline gap-2">
                ...
                <span class="text-sm text-gray-400">شامل ضريبة {{ $product['tax'] }}%</span>
            </div>
            <p class="mt-4 text-sm text-gray-600 leading-relaxed">
                باقة أنيقة من الورود الطبيعية الطازجة مناسبة لجميع المناسبات، منسّقة بعناية بأيدي خبراء التنسيق لدينا لتمنح لمسة جمالية مميزة.
            </p>
            <dl class="mt-6 grid grid-cols-2 gap-4 text-sm">
                <div>
                    <dt class="text-gray-400">رمز المنتج SKU</dt>
                    <dd class="mt-0.5 font-medium text-gray-800">{{ $product['sku'] }}</dd>
                </div>
                <div>
                    <dt class="text-gray-400">الباركود</dt>
                    <dd class="mt-0.5 font-medium text-gray-800">{{ $product['barcode'] }}</dd>
                </div>
                <div>
                    <dt class="text-gray-400">سعر التكلفة</dt>
                    <dd class="mt-0.5 font-medium text-gray-800">{{ \App\Support\Demo::money($product['cost']) }}</dd>
                </div>
                <div>
                    <dt class="text-gray-400">حد التنبيه</dt>
                    <dd class="mt-0.5 font-medium text-gray-800">{{ $product['alert'] }} قطعة</dd>
                </div>
            </dl>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <x-stat-card label="الكمية المتوفرة" :value="$product['qty'] . ' قطعة'" icon="package" color="primary" />
        <x-stat-card label="إجمالي المبيعات" value="342 قطعة" icon="shopping-cart" trend="+12%" :up="true" color="success" />
        <x-stat-card label="سعر التكلفة" :value="\App\Support\Demo::money($product['cost'])" icon="tag" color="info" />
        <x-stat-card label="هامش الربح" :value="$marginPct . '%'" :trend="\App\Support\Demo::money($margin)" :up="true" color="secondary" />
    </div>

    {{-- حركة المخزون --}}
    <div>
        <div class="flex items-center justify-between mb-3">
            <h3 class="font-bold text-gray-800">حركة المخزون</h3>
            <a href="{{ route('admin.inventory.movements') }}" class="text-sm text-primary-600 hover:text-primary-700 font-medium">عرض الكل</a>
        </div>
        <x-table :headers="['نوع الحركة', 'الكمية', 'الموظف', 'التاريخ']">
            @foreach (array_slice(array_filter(\App\Support\Demo::movements(), fn ($m) => $m['product'] === $product['name']), 0, 6) as $m)
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 font-medium text-gray-800 whitespace-nowrap">{{ $m['type'] }}</td>
