<x-layouts::admin title="أمر شراء جديد">

    <x-page-header title="أمر شراء جديد" subtitle="حدّد المورّد والأصناف المطلوبة"
        :breadcrumbs="['الرئيسية' => route('admin.dashboard'), 'أوامر الشراء' => route('admin.purchases.index'), 'جديد' => '#']" />
    ...
    <form method="POST" action="{{ route('admin.purchases.store') }}"
        x-data='{
            ...
            fmt(v) { return v.toLocaleString("ar", { minimumFractionDigits: 3, maximumFractionDigits: 3 }) + " ر.ع"; },
        }'>
        @csrf
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-4">
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                    <h3 class="font-bold text-gray-800 mb-4">الأصناف</h3>
                    <div class="space-y-3">
                        <template x-for="(item, i) in items" :key="i">
                            <div class="grid grid-cols-12 gap-2 items-end">
                                <div class="col-span-12 sm:col-span-5">
                                    <label class="block text-xs text-gray-500 mb-1" x-show="i===0">المنتج</label>
                                    ...
                                    <input type="text" :name="`items[${i}][name]`" x-model="item.name" placeholder="اسم الصنف" class="mt-1 w-full rounded-lg border-gray-200 text-sm" required />
                                </div>
                                <div class="col-span-5 sm:col-span-3">
                                    <label class="block text-xs text-gray-500 mb-1" x-show="i===0">تكلفة الوحدة</label>
                                ...
                                <div class="col-span-4 sm:col-span-2">
                                    <label class="block text-xs text-gray-500 mb-1" x-show="i===0">الكمية</label>
                    ...
                    <button type="button" @click="addRow()" class="mt-4 inline-flex items-center gap-1.5 text-sm text-primary-600 hover:text-primary-700 font-medium"><x-icon name="plus" class="w-4 h-4" /> إضافة صنف</button>
                </div>
            </div>
