<x-layouts::admin title="الإعدادات">
    ...
    <div x-data="{ tab: 'business' }" class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-2 lg:sticky lg:top-6">
                @php
                    $tabs = [
                        ...
                        'language' => ['label' => 'اللغة', 'icon' => 'languages'],
        ...
        {{-- اللغة (نموذج مستقل) --}}
        <div x-show="tab === 'language'" x-cloak class="lg:col-span-3 lg:col-start-2">
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <h3 class="text-lg font-bold text-gray-800 mb-2">لغة النظام</h3>
                <p class="text-sm text-gray-500 mb-5">تُطبَّق على واجهة لوحة التحكم ويتغيّر معها اتجاه الصفحة.</p>

                @php $currentLocale = app()->getLocale(); @endphp
                <form method="POST" action="{{ route('admin.language.update') }}" class="space-y-4">
                    @csrf
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach ([['ar', 'العربية', 'من اليمين إلى اليسار (RTL)'], ['en', 'English', 'Left to right (LTR)']] as [$code, $label, $hint])
                    ...
                    <div class="flex justify-end">
                        <x-button variant="primary" size="md" icon="save" type="submit">حفظ اللغة</x-button>
                    </div>
                </form>
            </div>
        </div>
