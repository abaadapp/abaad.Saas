<x-layouts::auth title="تسجيل الدخول">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-sm text-gray-500 mt-1">نظام نقاط البيع متعدد المتاجر</p>
        </div>

        {{-- البطاقة --}}
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8">
            <h2 class="text-lg font-bold text-gray-800 mb-1">مرحبًا بعودتك 👋</h2>
            <p class="text-sm text-gray-500 mb-6">سجّل الدخول للمتابعة إلى لوحة التحكم</p>
            ...
            <form action="{{ route('login.attempt') }}" method="POST" class="space-y-4"
                ...
                @csrf
                <x-input label="البريد الإلكتروني" name="email" type="email" icon="mail" placeholder="you@example.com" value="{{ old('email', 'admin@abadpos.com') }}" required />

                <div x-data="{ show: false }">
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-1.5">كلمة المرور <span class="text-danger-500">*</span></label>
                    <div class="relative">
                ...
                <x-button type="submit" variant="primary" class="w-full" x-bind:disabled="loading">
                    ...
                    <span x-show="loading" x-cloak class="flex items-center gap-2">
                        <x-icon name="loader-circle" class="w-4 h-4 animate-spin" /> جارٍ الدخول…
                    </span>
                </x-button>
            </form>
            ...
            <div class="mt-6">
                <div class="relative flex items-center">
                    ...
                    <span class="mx-3 text-xs text-gray-400">دخول تجريبي سريع</span>
                    <div class="flex-grow border-t border-gray-100"></div>
                </div>
                <div class="mt-4 space-y-2">
                    <a href="{{ route('demo.login', 'super-admin') }}" class="flex items-center gap-3 w-full p-3 rounded-xl border border-gray-200 hover:border-primary-300 hover:bg-primary-50/50 transition group">
                        ...
                        <div class="text-right flex-1">
                            <p class="text-sm font-semibold text-gray-800">مدير المنصة</p>
                            <p class="text-xs text-gray-400">Super Admin — إدارة منصة SaaS</p>
                        </div>
                    ...
                    <a href="{{ route('demo.login', 'admin') }}" class="flex items-center gap-3 w-full p-3 rounded-xl border border-gray-200 hover:border-secondary-300 hover:bg-secondary-50/50 transition group">
                        ...
                        <div class="text-right flex-1">
                            <p class="text-sm font-semibold text-gray-800">صاحب النشاط</p>
                            <p class="text-xs text-gray-400">Admin — إدارة المتجر</p>
                        </div>
                    ...
                    <a href="{{ route('demo.login', 'pos') }}" class="flex items-center gap-3 w-full p-3 rounded-xl border border-gray-200 hover:border-success-300 hover:bg-success-50/50 transition group">
                        ...
                        <div class="text-right flex-1">
                            <p class="text-sm font-semibold text-gray-800">الموظف / الكاشير</p>
                            <p class="text-xs text-gray-400">Employee POS — نقطة البيع</p>
                        </div>
        ...
        <p class="text-center text-xs text-gray-400 mt-6">© 2026 Abad POS — جميع الحقوق محفوظة</p>
    </div>
</x-layouts::auth>
