{{--
    ...
    المتغيّرات: `$items` قائمةُ `['name','qty','unit','total','note','custom']` ·
            ...
            <tr>
                ...
                <td class="bidi item" dir="{{ \App\Support\Paper::dirOf($item['name']) }}">
                    ...
                    {{--
                        ...
                        تسميةٌ ثمّ قيمة، ولا يُخترع لها عمود: عمودٌ خامس لا
                        يُملأ إلّا في بندٍ من كلّ عشرين يترك الورقةَ بياضًا
                        في تسعةَ عشر. وهي بمقاس `note` نفسِه لأنها من جنسه —
                        بيانٌ تابعٌ للسطر لا سطرٌ مستقلّ.

                        وتُرسَل جاهزةً من `DocumentPaper` — الظاهرةُ منها
                        وحدها، والداخليُّ مطروحٌ قبل أن يبلغ القالب. فالقالبُ
                        لا يقرّر ما يُفشى وما يُخفى.
                    --}}
                    @foreach ($item['custom'] ?? [] as $line)
                        {{--
                            والاتّجاهُ يُقاس من السطر كلِّه ابتداءً بتسميته، لا
                            من قيمته وحدها: التسميةُ أوّلُ ما يُقرأ، فهي التي
                            تقرّر من أيّ طرفٍ يبدأ السطر. ولو قِيس بالقيمة
                            لَخرج «Recipient: أحمد» مقلوبًا في متجرٍ إنجليزيّ
                            يكتب زبائنه بالعربية — التسميةُ تُدفع إلى الطرف
                            الآخر وتُقرأ النقطتان قبلها.
                        --}}
                        <div dir="{{ \App\Support\Paper::dirOf($line['label'].' '.$line['value']) }}" class="bidi lbl2">{{ $line['label'] }}: {{ $line['value'] }}</div>
                    @endforeach
                </td>
                @if ($ordered)
