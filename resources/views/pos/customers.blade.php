<x-layouts::pos title="العملاء">
    ...
    <div class="h-full overflow-y-auto p-4 sm:p-6" x-data="{ q: '' }">
        ...
        <x-modal name="new-customer" title="إضافة عميل جديد" maxWidth="max-w-md">
            <div class="space-y-4">
                <x-input label="الاسم الكامل" name="ncc-name" placeholder="اسم العميل" icon="user" :required="true" />
                <x-input label="رقم الهاتف" name="ncc-phone" type="tel" placeholder="+968 9xxxxxxx" icon="phone" :required="true" />
                <x-input label="البريد الإلكتروني" name="ncc-email" type="email" placeholder="example@mail.com" icon="mail" />
                <x-input label="العنوان" name="ncc-address" placeholder="المدينة / المنطقة" icon="map-pin" />
            </div>
            <x-slot:footer>
