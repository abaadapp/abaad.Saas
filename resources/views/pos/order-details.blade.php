            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <h1 class="text-xl font-bold text-gray-800">طلب {{ $order['id'] }}</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 max-w-sm mr-auto ml-0 w-full">
                    ...
                    <div class="space-y-2.5 text-sm">
                        <div class="flex justify-between text-gray-600"><span>المجموع الفرعي</span><span class="font-medium text-gray-800">{{ \App\Support\Demo::money($order['subtotal']) }}</span></div>
                        <div class="flex justify-between text-gray-600"><span>الخصم</span><span class="text-danger-500">- {{ \App\Support\Demo::money($order['discount']) }}</span></div>
                        <div class="flex justify-between text-gray-600"><span>الضريبة</span><span class="font-medium text-gray-800">{{ \App\Support\Demo::money($order['tax']) }}</span></div>
                        <div class="flex justify-between text-gray-600"><span>رسوم التوصيل</span><span class="font-medium text-gray-800">{{ \App\Support\Demo::money($order['delivery']) }}</span></div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                    ...
                    <div class="space-y-2.5 text-sm">
                        <div class="flex items-center justify-between text-gray-600"><span>وسيلة الدفع</span><span class="font-medium text-gray-800">{{ $order['payment'] }}</span></div>
                        <div class="flex justify-between text-gray-600"><span>حالة الدفع</span><x-badge type="success">{{ $order['payment_status'] }}</x-badge></div>
                        <div class="flex justify-between text-gray-600"><span>الموظف</span><span class="font-medium text-gray-800">{{ $order['employee'] }}</span></div>
                        <div class="flex justify-between text-gray-600"><span>الفرع</span><span class="font-medium text-gray-800">{{ $order['branch'] }}</span></div>
                    </div>
                </div>
            </div>
        </div>
