<x-layouts::super-admin :title="__('لوحة التحكم')">

    <x-page-header :title="__('لوحة التحكم')" :subtitle="__('نظرة عامة على أداء منصة Abad POS')">
        <x-slot:actions>
            <span class="inline-flex items-center gap-1.5 text-xs text-gray-500 bg-gray-50 border border-gray-100 rounded-full px-3 py-1.5">
                ...
                <span data-stat-updated>{{ __('مباشر') }}</span>
            </span>
            <div class="w-40">
                <x-select name="period" :options="['today' => __('اليوم'), 'week' => __('الأسبوع'), 'month' => __('الشهر'), 'year' => __('السنة')]" selected="month" />
            </div>
    ...
    <div x-data="dashboardGrid('super')" class="mb-6">
        <div class="flex flex-wrap items-center justify-end gap-2 mb-3">
            <div x-show="editing" x-cloak class="flex items-center gap-1.5 flex-wrap ml-auto mr-0">
                <span class="text-xs text-gray-400">{{ __('المخفية:') }}</span>
                ...
                <span x-show="!hidden.length" class="text-xs text-gray-300">{{ __('لا شيء') }}</span>
            </div>
            <button type="button" @click="toggleEdit()" :class="editing ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-gray-600 border-gray-200'" class="inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors">
                ...
                <span x-text="editing ? @js(__('تم')) : @js(__('تخصيص اللوحة'))"></span>
            </button>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4"
            ...
            @foreach (\App\Support\Demo::superStats() as $s)
                <x-stat-card :label="__($s['label'])" :value="$s['value']" :icon="$s['icon']" :trend="$s['trend']" :up="$s['up']" :color="$s['color']" />
            @endforeach
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-bold text-gray-800">{{ __('الإيرادات الشهرية') }}</h3>
                <span class="text-xs text-gray-400">{{ __('آخر 12 شهرًا') }}</span>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-bold text-gray-800">{{ __('نمو الشركات') }}</h3>
                <span class="text-xs text-gray-400">{{ __('شركات جديدة شهريًا') }}</span>
            </div>
            ...
            <div x-data='apexChart({
                ...
                series: [{ name: @json(__('شركات جديدة')), data: @json($growthSeries['data']) }],
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
        ...
        <div class="xl:col-span-2">
            <div class="flex items-center justify-between mb-3">
                <h3 class="font-bold text-gray-800">{{ __('آخر الشركات المسجلة') }}</h3>
                <a href="{{ route('super-admin.businesses.index') }}" class="text-sm text-primary-600 hover:text-primary-700 font-medium">{{ __('عرض الكل') }}</a>
            </div>
            <x-table :headers="[__('الشركة'), __('النوع'), __('المالك'), __('الباقة'), __('الحالة'), __('تاريخ التسجيل'), '']">
                @foreach (array_slice(\App\Support\Demo::businesses(), 0, 6) as $b)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            <x-button variant="ghost" size="sm" icon="eye" :href="route('super-admin.businesses.show', $b['id'])">{{ __('عرض') }}</x-button>
                        </td>
                    </tr>
                @endforeach
            </x-table>
        ...
        <div>
            <div class="flex items-center justify-between mb-3">
                <h3 class="font-bold text-gray-800">{{ __('أحدث الأنشطة') }}</h3>
            </div>
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <ul class="space-y-4">
    ...
    <div>
        <div class="flex items-center justify-between mb-3">
            <h3 class="font-bold text-gray-800">{{ __('اشتراكات ستنتهي قريبًا') }}</h3>
            <a href="{{ route('super-admin.subscriptions.index') }}" class="text-sm text-primary-600 hover:text-primary-700 font-medium">{{ __('إدارة الاشتراكات') }}</a>
        </div>
        <x-table :headers="[__('الشركة'), __('الباقة'), __('تاريخ الانتهاء'), __('المبلغ'), __('الحالة')]">
            @foreach (array_slice(\App\Support\Demo::subscriptions(), 0, 5) as $sub)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-gray-600 whitespace-nowrap">{{ __($sub['plan']) }}</td>
                    ...
                    <td class="px-4 py-3"><x-badge :text="__($sub['status'])" /></td>
                </tr>
            @endforeach
        </x-table>
    </div>
