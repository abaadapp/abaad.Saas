<x-layouts::super-admin :title="__('إضافة محل ورود')">

    <x-page-header :title="__('إضافة محل ورود')" :subtitle="__('أدخل بيانات محل الورود الجديد')"
        :breadcrumbs="[__('الرئيسية') => route('super-admin.dashboard'), __('محلات الورود') => route('super-admin.flower-shops.index'), __('إضافة محل') => '#']">
    </x-page-header>
    ...
    <form method="POST" action="{{ route('super-admin.flower-shops.store') }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <x-input :label="__('اسم المحل')" name="name" :placeholder="__('مثال: زهرة مسقط')" :required="true" />
                <x-input :label="__('اسم المالك')" name="owner" :placeholder="__('الاسم الكامل')" :required="true" />
                ...
                <x-input :label="__('عدد الفروع')" name="branches" type="number" value="1" :placeholder="__('عدد الفروع')" />
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <x-select :label="__('الباقة')" name="plan" :placeholder="__('اختر الباقة...')"
                    :options="['أساسية' => 'أساسية', 'احترافية' => 'احترافية', 'مؤسسات' => 'مؤسسات']" />
                <x-select :label="__('حالة الحساب')" name="status" :placeholder="__('اختر الحالة...')" selected="نشط"
                    :options="['نشط' => __('نشط'), 'منتهي' => __('منتهي')]" />
                <x-input :label="__('تاريخ البداية')" name="start" type="date" />
                <x-input :label="__('تاريخ الانتهاء')" name="end" type="date" />
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <label class="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center cursor-pointer hover:border-secondary-300 hover:bg-secondary-50/30 transition">
                ...
                <span class="text-sm font-medium text-gray-700">{{ __('اسحب الشعار هنا أو انقر للرفع') }}</span>
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <x-button variant="secondary" type="submit" icon="check">{{ __('حفظ المحل') }}</x-button>
        </div>
    </form>
