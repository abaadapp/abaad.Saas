<x-layouts::super-admin title="تفاصيل محل الورود">
    ...
    <x-page-header title="تفاصيل محل الورود" :subtitle="$shop['name']"
            ...
            <div class="flex flex-col sm:flex-row sm:items-end gap-4">
                ...
                <div class="flex-1 sm:pb-1">
                    ...
                    <p class="mt-1 text-sm text-gray-500 flex items-center gap-3 flex-wrap">
                        ...
                        <span class="flex items-center gap-1"><x-icon name="layers" class="w-4 h-4" /> باقة {{ $shop['plan'] }}</span>
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <x-stat-card label="الفروع" :value="$shop['branches']" icon="git-branch" color="primary" />
        <x-stat-card label="الموظفون" :value="$shop['employees']" icon="users" color="info" />
        <x-stat-card label="المنتجات" :value="$shop['products']" icon="flower" color="secondary" />
        <x-stat-card label="الطلبات" :value="$shop['orders']" icon="shopping-bag" color="success" />
    </div>

    {{-- بطاقات المعلومات --}}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <h3 class="font-bold text-gray-800 mb-4">معلومات المحل</h3>
            <dl class="space-y-3 text-sm">
                <div class="flex justify-between gap-3"><dt class="text-gray-500">الاسم</dt><dd class="font-medium text-gray-800">{{ $shop['name'] }}</dd></div>
                <div class="flex justify-between gap-3"><dt class="text-gray-500">المدينة</dt><dd class="font-medium text-gray-800">{{ $shop['city'] }}</dd></div>
                <div class="flex justify-between gap-3"><dt class="text-gray-500">عدد الفروع</dt><dd class="font-medium text-gray-800">{{ $shop['branches'] }}</dd></div>
                <div class="flex justify-between gap-3"><dt class="text-gray-500">إجمالي المبيعات</dt><dd class="font-medium text-gray-800">{{ \App\Support\Demo::money($shop['sales']) }}</dd></div>
            </dl>
        </div>
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <h3 class="font-bold text-gray-800 mb-4">بيانات المالك</h3>
            <dl class="space-y-3 text-sm">
                <div class="flex justify-between gap-3"><dt class="text-gray-500">الاسم</dt><dd class="font-medium text-gray-800">{{ $shop['owner'] }}</dd></div>
                <div class="flex justify-between gap-3"><dt class="text-gray-500">الهاتف</dt><dd class="font-medium text-gray-800" dir="ltr">+968 91234567</dd></div>
                <div class="flex justify-between gap-3"><dt class="text-gray-500">البريد</dt><dd class="font-medium text-gray-800" dir="ltr">info@flower.com</dd></div>
            </dl>
        </div>
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <h3 class="font-bold text-gray-800 mb-4">الاشتراك الحالي</h3>
            <dl class="space-y-3 text-sm">
                <div class="flex justify-between gap-3"><dt class="text-gray-500">الباقة</dt><dd class="font-medium text-gray-800">{{ $shop['plan'] }}</dd></div>
    ...
    <div x-data="{ tab: 'branches' }" class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="border-b border-gray-100 px-4">
            <nav class="flex gap-1 -mb-px overflow-x-auto">
                ...
                <button type="button" @click="tab = 'employees'"
                    ...
                    class="px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition">الموظفون</button>
                <button type="button" @click="tab = 'products'"
                    ...
                    class="px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition">المنتجات</button>
                <button type="button" @click="tab = 'sales'"
                    ...
                    class="px-4 py-3 text-sm font-medium border-b-2 whitespace-nowrap transition">المبيعات</button>
        ...
        <div class="p-5">
                ...
                <x-table :headers="['الفرع', 'المدينة', 'المدير', 'عدد الموظفين', 'الحالة']">
                    @for ($i = 1; $i <= $shop['branches']; $i++)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 font-medium text-gray-800 whitespace-nowrap">{{ $i === 1 ? 'الفرع الرئيسي' : 'فرع ' . $shop['city'] . ' ' . $i }}</td>
            ...
            <div x-show="tab === 'employees'" x-cloak>
                <x-table :headers="['الموظف', 'الدور', 'الفرع', 'الهاتف', 'المبيعات', 'الحالة']">
                    @foreach (\App\Support\Demo::employees() as $emp)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-gray-600 whitespace-nowrap">{{ $emp['role'] }}</td>
                            ...
                            <td class="px-4 py-3"><x-badge :text="$emp['status']" /></td>
                        </tr>
                    @endforeach
                </x-table>
            </div>
            ...
            <div x-show="tab === 'sales'" x-cloak>
                <div class="flex items-center justify-between mb-4">
                    <h4 class="font-semibold text-gray-800">مبيعات آخر 12 شهرًا</h4>
